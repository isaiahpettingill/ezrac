...
| 21. Pointers | Implemented | Address-of, dereference, arithmetic, casts, comparisons, nulls, mutability, and target widths are tested. TBIR tracks named global, MMIO, and embed object provenance for conservative memory optimization. | Pointer-derived range and escape analysis remains future work. |
...
| 24–25. Operators/casts | Implemented | Constant/runtime arithmetic, bitwise, shifts, comparisons, signed behavior, pointers, and casts are tested. TBIR rewrites power-of-two multiplication to shifts, while 8086, MC6800, TMS9900, and DCPU-16 select CPU-specific signed arithmetic and shift forms. | None known. |
...
| 27–29. Functions and ABI | Partial | Parameters, spills, recursion, returns, frames, inline/naked functions, and extern calls are tested on current full source backends, including MOS 6502; AVR covers scalar and aggregate parameters, recursion-safe calls, return values, inline/naked functions, and interrupts. | Runtime validation remains target-specific. |
...
| 40. Compiler pipeline | Partial | AST → HIR → TBIR → assembly → assembler → layout is explicit and dumpable; all source backends receive the TBIR-transformed program. Safe TBIR optimization tests cover scalar simplification, typed immutable-local propagation, local common-subexpression cleanup, pure scalar and named global-read loop-invariant hoisting, cross-platform explicit inlining, direct tail-recursion conversion, and compatible eZ80-family sibling tail calls. | Pointer-range, cache-aware, and lower-level passes remain future work. |
...
5. Extend typed object provenance to pointer ranges and target-benefit-driven cache/locality passes.
