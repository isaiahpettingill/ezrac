...
- FED: run `ezrac install-syntax fed`. The installer finds `fed.syn` through `FED_HOME`, a `fed` executable on `PATH`, or the current directory, then updates only its marked EZRA section. For a manual install, append `editors/fed/ezra.syn` to the `fed.syn` beside the FED executable.
