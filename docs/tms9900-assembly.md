...
The initial source ABI evaluates scalar values in `R0`, uses `R1`/`R2` as scratch registers, and keeps arguments and call links in compiler-owned expansion RAM. Calls also place their first ten arguments in `R0` through `R9`, giving naked platform wrappers a stable register ABI. Recursive functions are not supported. Source code currently supports 8- and 16-bit scalar variables, calls, basic arithmetic including native unsigned and signed 16-bit multiplication, bitwise operations, comparisons, loops, MMIO, inline assembly, and embedded byte data; arrays, structs, 24-bit values, shifts, division, and remainder remain unsupported by this backend.
