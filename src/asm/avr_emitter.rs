...
pub fn emit_avr_assembly_with_options(
    ...
    if !options.cpu.is_avr() {
        return Err(Diagnostic::new("AVR emitter requires an AVR target"));
    }
    if options.arduboy_executable && options.cpu != CpuFamily::AvrMega {
        return Err(Diagnostic::new(
            "Arduboy source generation requires the ATmega32U4 megaAVR target",
        ));
    }
    if options.ram_base.get() < POINTER_SCRATCH_BYTES {
        return Err(Diagnostic::new(
            "AVR RAM base must leave four bytes for indirect pointer scratch storage",
        ));
    }
...
impl Emitter {
    ...
    fn emit(mut self, program: &Program) -> Result<String, Diagnostic> {
        ...
        self.line(&format!("; family: {}", self.cpu.as_str()));
        ...
        self.line("    cli");
        self.line(&format!("    ldi r16, {:02X}h", self.stack_top & 0xFF));
        self.line("    out 3Dh, r16");
        self.line(&format!(
            "    ldi r16, {:02X}h",
            (self.stack_top >> 8) & 0xFF
        ));
    ...
    fn emit_return_two(&mut self, first: &Expr, second: &Expr) -> Result<(), Diagnostic> {
        ...
        self.set_zp_from_storage(self.pointer_scratch, pointer);
    ...
    fn emit_call(
        ...
        match name.as_str() {
            ...
            "mem.poke8" | "ezra.mem.poke8" => {
                ...
                self.line("    ldy #$00");
                self.line(&format!("    sta (${:04X}),y", self.pointer_scratch));
                return Ok(());
            }
    ...
    fn intrinsic_memory_range(
        ...
        self.set_zp_from_storage(self.pointer_scratch, values[1]);
        self.set_zp_from_storage(self.pointer_scratch + 2, values[0]);
        self.native_line(
            "    lds r30, 0100h\n    lds r31, 0101h\n    lds r26, 0102h\n    lds r27, 0103h",
        );
    ...
    fn intrinsic_compare(&mut self, values: &[Storage]) {
        self.set_zp_from_storage(self.pointer_scratch, values[0]);
        self.set_zp_from_storage(self.pointer_scratch + 2, values[1]);
        self.native_line(
            "    lds r30, 0100h\n    lds r31, 0101h\n    lds r26, 0102h\n    lds r27, 0103h",
        );
        let length = values[2];
    ...
    fn emit_memcpy(&mut self, args: &[Expr]) -> Result<(), Diagnostic> {
        ...
        self.set_zp_from_storage(self.pointer_scratch, source);
        self.set_zp_from_storage(self.pointer_scratch + 2, destination);
        self.line(&format!("{loop_label}:"));
        self.jump_storage_zero(length, 2, &done);
        self.line("    ldy #$00");
        self.line(&format!("    lda (${:04X}),y", self.pointer_scratch));
        self.line(&format!("    sta (${:04X}),y", self.pointer_scratch + 2));
        self.increment_zp(self.pointer_scratch);
        self.increment_zp(self.pointer_scratch + 2);
    ...
    fn add_index_to_pointer(&mut self, index: &Expr, element_size: u32) -> Result<(), Diagnostic> {
        ...
        self.sta(self.pointer_scratch + 1);
    ...
    fn native_line(&mut self, line: &str) {
        let mut relocated = line.to_owned();
        for offset in 0..POINTER_SCRATCH_BYTES {
            relocated = relocated.replace(
                &format!("{:04X}h", 0x0100 + offset),
...
fn translate_avr_line(line: &str, pointer_scratch: u32) -> String {
    ...
    match op {
        ...
        "lda" if arg.starts_with('(') => format!(
            "    lds r30, {pointer_scratch:04X}h\n    lds r31, {:04X}h\n    add r30, r17\n    adc r31, r1\n    ld r16, z\n    tst r16",
            pointer_scratch + 1
        ),
        "lda" => format!("{indent}lds r16, {}\n    tst r16", direct(arg)),
        "sta" if arg.starts_with('(') => format!(
            "    lds r30, {pointer_scratch:04X}h\n    lds r31, {:04X}h\n    add r30, r17\n    adc r31, r1\n    st z, r16",
            pointer_scratch + 1
        ),
