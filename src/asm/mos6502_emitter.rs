...
impl Emitter {
    ...
    fn emit(mut self, program: &Program) -> Result<String, Diagnostic> {
        ...
        self.line(if self.cpu == CpuFamily::Wdc65C816 {
            "; target: WDC 65C816 (native mode, byte ABI)"
        } else {
            "; target: MOS 6502"
        });
        ...
        if self.cpu == CpuFamily::Wdc65C816 {
            // Reset begins in emulation mode. The byte-oriented backend keeps
            // M and X set after creating a native 16-bit hardware stack.
            self.line("    clc");
            self.line("    xce");
            self.line("    rep #$10");
            self.line("    ldx #$1FFF");
    ...
    fn increment(&mut self, storage: Storage, width: u8) {
        ...
        for offset in 0..u32::from(width) {
            ...
            if self.cpu == CpuFamily::Wdc65C816 && address > u16::MAX.into() {
                ...
                self.lda(address);
                self.line(&format!("    adc #${:02X}", u8::from(offset == 0)));
                self.sta(address);
            } else {
                self.line(&format!("    inc ${address:04X}"));
            }
            self.branch_long("bne", &done);
        }
        self.line(&format!("{done}:"));
