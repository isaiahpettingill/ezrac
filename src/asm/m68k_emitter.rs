...
impl Emitter {
    ...
    fn emit_function_pointer_trampoline(&mut self, function: &Function) -> Result<(), Diagnostic> {
        ...
        self.line(&format!("    jsr ({}).l", function_label(&function.name)));
        self.line("    rts");
        Ok(())
    }
        ...
        match expr {
            ...
            Expr::AddressOf(name) => {
                if let Some(function_type) = self.function_value_type(name) {
                    ...
                    self.line(&format!("    move.l #{},d0", function_pointer_label(name)));
    ...
    fn emit_call(
        ...
        let direct_signature = self
            ...
            .cloned();
        let (signature, indirect_binding, argument_slots) =
            if let Some(signature) = direct_signature {
                let argument_slots = signature.argument_slots.clone();
                (signature, None, argument_slots)
            } else {
                if path.len() != 1 {
                ...
                (
                    crate::tbir::model::FunctionSignature {
                        ...
                        argument_slots: argument_slots.clone(),
                    },
                    Some(binding),
                    argument_slots,
                )
        ...
        for ((arg, ty), slot) in args.iter().zip(&signature.params).zip(argument_slots) {
            let ty = self.model.resolved_type(ty)?;
...
fn function_pointer_label(name: &str) -> String {
    format!("__ezra_fn_ptr_{}", sanitize(name))
}
