use crate::{
    asm::{
        AssemblyOptions,
        comments::{stmt_summary, with_readability_comments},
        data::{ByteLiteralStyle, byte_data_lines, terminated_text_data_line},
        reachability::{RoutineProfile, strip_unreachable_generated_routines},
    },
    ast::{
        AccessPath, AccessSegment, AssignOp, BinaryOp, Declaration, Expr, Function, Place, Program,
        Stmt, Type, UnaryOp,
    },
    compat::prelude::*,
    diagnostic::Diagnostic,
    hir::HirProgram,
    intrinsics::{BitsIntrinsic, IntIntrinsic, IntrinsicOperation, MemIntrinsic},
    regalloc::{
        Location, PhysReg, PhysicalRegister, RegClass, RegUnit, RegisterClass, RegisterUnit,
        SpillClass, SpillClassId, Target,
        source::{SourceLocal, allocate_source_locals},
    },
    target::CpuFamily,
    tbir::{
        TbirProgram,
        model::{SemanticModel, Storage},
    },
};

/// Emit code for the generic 68000 target.
///
/// Values are evaluated in D0, with D1 as the binary-operation scratch
/// register. Scalar two-result functions return their first value in D0 and
/// their second value in D1. Non-naked functions use A6 as a per-invocation
/// frame pointer; parameters are positive frame offsets and spilled locals and
/// compiler scratch are negative frame offsets. Calls marshal arguments in a
/// caller-owned stack area.
pub fn emit_m68k_assembly_with_options(
    program: &Program,
    options: AssemblyOptions,
) -> Result<String, Diagnostic> {
    if options.cpu != CpuFamily::M68k {
        return Err(Diagnostic::new("M68k emitter requires an m68k target"));
    }
    let hir = HirProgram::from_ast(program)?;
    let (lowered_program, source_comments) = if contains_function_pointer_program(program) {
        (program.clone(), Vec::new())
    } else {
        let tbir = TbirProgram::lower(&hir, program, &options)?;
        (tbir.lowered_program, tbir.source_comments)
    };
    let model = SemanticModel::from_program_with_native_int_widths(
        &lowered_program,
        24,
        options.ram_base.get(),
        options.rodata_base.get(),
        options.asset_base.get(),
        CpuFamily::M68k.capabilities().native_int_widths,
    )?;
    Emitter::new(model, options.clone())?
        .emit(&lowered_program)
        .map(|asm| {
            let asm = if options
                .optimization
                .is_enabled(crate::optimization::OptimizationPass::RedundantRegisterCopies)
            {
                crate::asm::copy_cleanup::remove_redundant_register_copies(&asm, options.cpu)
            } else {
                asm
            };
            let asm = if options
                .optimization
                .is_enabled(crate::optimization::OptimizationPass::DeadCodeElimination)
            {
                strip_unreachable_generated_routines(&asm, RoutineProfile::M68k)
            } else {
                asm
            };
            with_readability_comments(asm, program, &options, "m68k", &source_comments)
        })
}

#[derive(Clone, Copy)]
enum BindingLocation {
    Memory(Storage),
    AddressRegister(u8),
}

#[derive(Clone, Copy)]
enum MemoryRef {
    Absolute(u32),
    Frame(i32),
    Stack(i32),
}

impl MemoryRef {
    fn offset(self, offset: i32) -> Self {
        match self {
            Self::Absolute(address) => {
                if offset >= 0 {
                    Self::Absolute(address.saturating_add(offset as u32))
                } else {
                    Self::Absolute(address.saturating_sub(offset.unsigned_abs()))
                }
            }
            Self::Frame(base) => Self::Frame(base.saturating_add(offset)),
            Self::Stack(base) => Self::Stack(base.saturating_add(offset)),
        }
    }
}

#[derive(Clone)]
struct Binding {
    location: BindingLocation,
    ty: Type,
    volatile: bool,
}

impl Binding {
    fn memory(&self) -> Result<Storage, Diagnostic> {
        match self.location {
            BindingLocation::Memory(storage) => Ok(storage),
            BindingLocation::AddressRegister(register) => Err(Diagnostic::new(format!(
                "address-register local in a{register} does not have memory storage"
            ))),
        }
    }
}

#[derive(Clone)]
struct LoopLabels {
    continue_label: String,
    break_label: String,
}

struct Emitter {
    model: SemanticModel,
    options: AssemblyOptions,
    out: String,
    labels: usize,
    scopes: Vec<HashMap<String, Binding>>,
    loops: Vec<LoopLabels>,
    return_labels: Vec<String>,
    return_types: Vec<Option<Type>>,
    second_return_types: Vec<Option<Type>>,
    local_plans: Vec<HashMap<String, Binding>>,
    frame_storage_slots: Vec<Vec<(Storage, i32)>>,
    intrinsic_scratch: Vec<Storage>,
    aggregate_scratch: Vec<Option<Storage>>,
}

impl Emitter {
    fn new(model: SemanticModel, options: AssemblyOptions) -> Result<Self, Diagnostic> {
        Ok(Self {
            model,
            options,
            out: String::new(),
            labels: 0,
            scopes: Vec::new(),
            loops: Vec::new(),
            return_labels: Vec::new(),
            return_types: Vec::new(),
            second_return_types: Vec::new(),
            local_plans: Vec::new(),
            frame_storage_slots: Vec::new(),
            intrinsic_scratch: Vec::new(),
            aggregate_scratch: Vec::new(),
        })
    }

    fn emit(mut self, program: &Program) -> Result<String, Diagnostic> {
        self.line("; generated by ezrac");
        self.line("; target: Motorola 68000");
        self.line("section .text");
        self.line("__ezra_start:");
        self.line(&format!(
            "    move.l #${:06X},sp",
            self.options.stack_top.get()
        ));
        self.emit_static_initializers(program)?;
        self.line("    jsr (_main).l");
        self.line("__ezra_exit:");
        self.line("    bra __ezra_exit");
        for declaration in &program.declarations {
            if let Declaration::Function(function) = declaration {
                self.emit_function(function)?;
            }
        }
        let mut strings = self
            .model
            .strings
            .iter()
            .map(|(value, storage)| (storage.address, value.clone()))
            .collect::<Vec<_>>();
        strings.sort_by_key(|(address, _)| *address);
        if !strings.is_empty() {
            self.line("section .rodata");
            for (_, value) in strings {
                self.emit_text_data(&value);
            }
        }
        let mut embeds = self.model.embeds.values().cloned().collect::<Vec<_>>();
        embeds.sort_by_key(|embed| embed.storage.address);
        if !embeds.is_empty() {
            self.line("section .assets");
            for embed in embeds {
                self.emit_data_bytes(embed.bytes);
            }
        }
        Ok(self.out)
    }

    fn function_value_type(&self, name: &str) -> Option<Type> {
        self.model
            .functions
            .get(name)
            .filter(|signature| signature.second_return_type.is_none())
            .map(|signature| Type::Function {
                params: signature.params.clone(),
                return_type: signature.return_type.clone().map(Box::new),
            })
    }

    fn emit_static_initializers(&mut self, program: &Program) -> Result<(), Diagnostic> {
        for declaration in &program.declarations {
            match declaration {
                Declaration::Const(constant) if matches!(constant.ty, Type::Array { .. }) => {
                    let storage = self.model.globals[&constant.name];
                    let ty = self.model.resolved_type(&constant.ty)?;
                    self.emit_initializer(storage, &ty, &constant.value)?;
                }
                Declaration::Global(global) => {
                    let storage = self.model.globals[&global.name];
                    let ty = self.model.resolved_type(&global.ty)?;
                    self.emit_initializer(storage, &ty, &global.value)?;
                }
                _ => {}
            }
        }
        Ok(())
    }

    fn emit_function(&mut self, function: &Function) -> Result<(), Diagnostic> {
        let naked = function.attrs.iter().any(|attr| attr == "naked");
        let interrupt = function.attrs.iter().any(|attr| attr == "interrupt");
        if naked
            && function.body.iter().any(|stmt| {
                !matches!(stmt, Stmt::Asm { inputs, outputs, .. } if inputs.is_empty() && outputs.is_empty())
            })
        {
            return Err(Diagnostic::new(format!(
                "naked function `{}` may contain only asm blocks without operands",
                function.name
            )));
        }
        if interrupt
            && (!function.params.is_empty()
                || function.return_type.is_some()
                || function.second_return_type.is_some())
        {
            return Err(Diagnostic::new(format!(
                "interrupt function `{}` cannot have parameters or a return value",
                function.name
            )));
        }
        if function.name == "main" && function.second_return_type.is_some() {
            return Err(Diagnostic::new(
                "main cannot return two values because its startup caller has no second-result destination",
            ));
        }
        if function.second_return_type.is_some() && function.return_type.is_none() {
            return Err(Diagnostic::new(format!(
                "two-result function `{}` must have a first return type",
                function.name
            )));
        }
        if let Some(return_type) = &function.return_type {
            self.require_scalar(&self.model.resolved_type(return_type)?)?;
        }
        if let Some(second_return_type) = &function.second_return_type {
            self.require_scalar(&self.model.resolved_type(second_return_type)?)?;
            if !block_guarantees_two_result_return(&function.body) {
                return Err(Diagnostic::new(format!(
                    "missing two return values in function `{}`",
                    function.name
                )));
            }
        }
        let signature = self.model.functions[&function.name].clone();
        if naked {
            self.line(&format!("{}:", function_label(&function.name)));
            self.emit_block(&function.body)?;
            return Ok(());
        }

        let frame = plan_function_locals(function, &mut self.model)?;
        let return_label = self.next_label(&format!("{}_return", function.name));
        let mut frame_storage_slots = frame.storage_slots;
        let mut parameter_offset = 8_i32;
        for (param, slot) in function.params.iter().zip(&signature.argument_slots) {
            let ty = self.model.resolved_type(&param.ty)?;
            let slot_size = abi_slot_size(&self.model, &ty)?;
            frame_storage_slots.push((*slot, parameter_offset));
            parameter_offset = parameter_offset
                .checked_add(
                    i32::try_from(slot_size)
                        .map_err(|_| Diagnostic::new("M68k parameter frame is too large"))?,
                )
                .ok_or_else(|| Diagnostic::new("M68k parameter frame is too large"))?;
        }
        if parameter_offset > i32::from(i16::MAX) {
            return Err(Diagnostic::new(
                "M68k parameter displacement exceeds the signed frame range",
            ));
        }

        self.line(&format!("{}:", function_label(&function.name)));
        self.scopes.push(HashMap::new());
        self.return_labels.push(return_label.clone());
        self.local_plans.push(frame.locals);
        self.frame_storage_slots.push(frame_storage_slots);
        self.intrinsic_scratch.push(frame.intrinsic_scratch);
        self.aggregate_scratch.push(frame.aggregate_scratch);
        self.return_types.push(
            function
                .return_type
                .as_ref()
                .map(|ty| self.model.resolved_type(ty))
                .transpose()?,
        );
        self.second_return_types.push(
            function
                .second_return_type
                .as_ref()
                .map(|ty| self.model.resolved_type(ty))
                .transpose()?,
        );
        if interrupt {
            for register in (0..8)
                .map(|index| format!("d{index}"))
                .chain((0..7).map(|index| format!("a{index}")))
            {
                self.line(&format!("    move.l {register},-(sp)"));
            }
        }
        self.line(&format!("    link a6,#-{}", frame.frame_size));
        for (param, slot) in function.params.iter().zip(signature.argument_slots) {
            let ty = self.model.resolved_type(&param.ty)?;
            self.bind_memory(param.name.clone(), slot, ty)?;
        }
        self.emit_block(&function.body)?;
        self.line("    nop");
        self.line(&format!("{return_label}:"));
        self.line("    unlk a6");
        if interrupt {
            for register in (0..7)
                .rev()
                .map(|index| format!("a{index}"))
                .chain((0..8).rev().map(|index| format!("d{index}")))
            {
                self.line(&format!("    move.l (sp)+,{register}"));
            }
            self.line("    rte");
        } else {
            self.line("    rts");
        }
        self.second_return_types.pop();
        self.return_types.pop();
        self.aggregate_scratch.pop();
        self.intrinsic_scratch.pop();
        self.frame_storage_slots.pop();
        self.return_labels.pop();
        self.local_plans.pop();
        self.scopes.pop();
        Ok(())
    }

    fn emit_block(&mut self, body: &[Stmt]) -> Result<(), Diagnostic> {
        for stmt in body {
            self.emit_stmt(stmt)?;
        }
        Ok(())
    }

    fn emit_stmt(&mut self, stmt: &Stmt) -> Result<(), Diagnostic> {
        self.line(&format!("    ; source: {}", stmt_summary(stmt)));
        match stmt {
            Stmt::Let { name, ty, value } => {
                let ty = self.model.resolved_type(ty)?;
                let binding = self
                    .local_plans
                    .last()
                    .and_then(|locals| locals.get(name))
                    .cloned()
                    .ok_or_else(|| {
                        Diagnostic::new(format!("missing allocation for local `{name}`"))
                    })?;
                self.bind(name.clone(), binding.clone())?;
                match binding.location {
                    BindingLocation::Memory(storage) => {
                        self.emit_initializer(storage, &ty, value)?
                    }
                    BindingLocation::AddressRegister(register) => {
                        debug_assert!(matches!(ty, Type::Ptr(_)));
                        self.emit_expr(value, &ty)?;
                        self.line(&format!("    move.l d0,a{register}"));
                    }
                }
            }
            Stmt::Assign { target, op, value } => {
                let ty = self.place_type(target)?;
                if self.model.type_width(&ty).is_err() {
                    if *op != AssignOp::Set {
                        return Err(Diagnostic::new(
                            "compound assignment requires a scalar value",
                        ));
                    }
                    let storage = self
                        .aggregate_scratch
                        .last()
                        .and_then(Clone::clone)
                        .ok_or_else(|| {
                            Diagnostic::new("missing M68k aggregate scratch frame storage")
                        })?;
                    let size = self.model.type_size(&ty)?;
                    self.emit_initializer(storage, &ty, value)?;
                    self.store_aggregate_place(target, storage, size)?;
                    return Ok(());
                }
                if matches!(op, AssignOp::Shl | AssignOp::Shr)
                    && let Some(count) = self.constant_shift_count(value)
                    && self.emit_memory_shift_assignment(target, &ty, *op, count)?
                {
                    return Ok(());
                }
                if *op == AssignOp::Set {
                    self.emit_expr(value, &ty)?;
                } else if self.emit_local_single_bit_compound_assignment(target, &ty, *op, value)? {
                    return Ok(());
                } else {
                    self.load_place(target, &ty)?;
                    self.line(&format!("    move.{} d0,d1", size_suffix(&ty)?));
                    self.emit_expr(value, &ty)?;
                    self.line(&format!("    move.{} d0,d2", size_suffix(&ty)?));
                    self.line(&format!("    move.{} d1,d0", size_suffix(&ty)?));
                    self.emit_binary(assign_binary(*op), &ty)?;
                }
                self.store_place(target, &ty)?;
            }
            Stmt::If {
                condition,
                then_body,
                else_body,
            } => {
                let otherwise = self.next_label("if_else");
                let done = self.next_label("if_end");
                let false_label = if else_body.is_empty() {
                    &done
                } else {
                    &otherwise
                };
                if !self.emit_jump_if_false(condition, false_label)? {
                    self.emit_expr(condition, &Type::Named("bool".to_owned()))?;
                    self.line("    tst.b d0");
                    self.line(&format!("    beq {false_label}"));
                }
                self.emit_block(then_body)?;
                self.line(&format!("    bra {done}"));
                self.line(&format!("{otherwise}:"));
                self.emit_block(else_body)?;
                self.line(&format!("{done}:"));
            }
            Stmt::While { condition, body } => {
                let check = self.next_label("while_check");
                let done = self.next_label("while_end");
                self.loops.push(LoopLabels {
                    continue_label: check.clone(),
                    break_label: done.clone(),
                });
                self.line(&format!("{check}:"));
                if !self.emit_jump_if_false(condition, &done)? {
                    self.emit_expr(condition, &Type::Named("bool".to_owned()))?;
                    self.line("    tst.b d0");
                    self.line(&format!("    beq {done}"));
                }
                self.emit_block(body)?;
                self.line(&format!("    bra {check}"));
                self.line(&format!("{done}:"));
                self.loops.pop();
            }
            Stmt::Loop { body } => {
                let again = self.next_label("loop");
                let done = self.next_label("loop_end");
                self.loops.push(LoopLabels {
                    continue_label: again.clone(),
                    break_label: done.clone(),
                });
                self.line(&format!("{again}:"));
                self.emit_block(body)?;
                self.line(&format!("    bra {again}"));
                self.line(&format!("{done}:"));
                self.loops.pop();
            }
            Stmt::Break => self.jump_loop(false)?,
            Stmt::Continue => self.jump_loop(true)?,
            Stmt::Return(value) => {
                if self
                    .second_return_types
                    .last()
                    .and_then(Clone::clone)
                    .is_some()
                {
                    return Err(Diagnostic::new(
                        "two-result function must use `return first, second`",
                    ));
                }
                if let Some(value) = value {
                    let ty = self
                        .return_types
                        .last()
                        .and_then(Clone::clone)
                        .ok_or_else(|| Diagnostic::new("value return in void function"))?;
                    self.emit_expr(value, &ty)?;
                }
                let label = self
                    .return_labels
                    .last()
                    .expect("function return label")
                    .clone();
                self.line(&format!("    bra {label}"));
            }
            Stmt::ReturnTwo { first, second } => self.emit_return_two(first, second)?,
            Stmt::LetTwo {
                first_name,
                first_ty: _,
                second_name,
                second_ty: _,
                value,
            } => {
                let first_binding = self
                    .local_plans
                    .last()
                    .and_then(|locals| locals.get(first_name))
                    .cloned()
                    .ok_or_else(|| {
                        Diagnostic::new(format!("missing allocation for local `{first_name}`"))
                    })?;
                let second_binding = self
                    .local_plans
                    .last()
                    .and_then(|locals| locals.get(second_name))
                    .cloned()
                    .ok_or_else(|| {
                        Diagnostic::new(format!("missing allocation for local `{second_name}`"))
                    })?;
                self.require_scalar(&first_binding.ty)?;
                self.require_scalar(&second_binding.ty)?;
                self.bind(first_name.clone(), first_binding.clone())?;
                self.bind(second_name.clone(), second_binding.clone())?;
                let Expr::Call { path, args } = value else {
                    return Err(Diagnostic::new(
                        "two-result bindings require a direct two-result call",
                    ));
                };
                self.emit_two_result_call(path, args, &first_binding, &second_binding)?;
            }
            Stmt::Asm {
                inputs,
                outputs,
                lines,
                ..
            } => self.emit_inline_asm(inputs, outputs, lines)?,
            Stmt::Out { port, .. } => {
                return Err(Diagnostic::new(format!(
                    "M68k does not support separate port I/O `{port}`; use mmio instead"
                )));
            }
            Stmt::Expr(expr) => {
                if let Expr::Call { path, .. } = expr {
                    self.emit_call(path, expr_call_args(expr)?, None)?;
                } else {
                    let ty = self.expr_type(expr)?;
                    self.emit_expr(expr, &ty)?;
                }
            }
        }
        Ok(())
    }

    fn emit_initializer(
        &mut self,
        storage: Storage,
        ty: &Type,
        value: &Expr,
    ) -> Result<(), Diagnostic> {
        let resolved = self.model.resolved_type(ty)?;
        match (resolved, value) {
            (Type::Array { .. }, Expr::Ident(name)) => {
                let source = self.binding(name)?;
                self.copy(source.memory()?, storage, storage.size);
            }
            (Type::Array { element, len }, Expr::Array(values)) => {
                let element_size = self.model.type_size(&element)?;
                let len = u32::try_from(self.model.const_value(&len)?)
                    .map_err(|_| Diagnostic::new("invalid array length"))?;
                for index in 0..len {
                    let element_storage = Storage {
                        address: storage.address + index * element_size,
                        size: element_size,
                    };
                    if let Some(value) = values.get(index as usize) {
                        self.emit_initializer(element_storage, &element, value)?;
                    } else {
                        self.zero(element_storage);
                    }
                }
            }
            (Type::Named(name), Expr::StructInit { fields, .. })
                if self.model.structs.contains_key(&name) =>
            {
                self.zero(storage);
                let layout = self.model.structs[&name].clone();
                for (field_name, value) in fields {
                    let field = layout
                        .fields
                        .get(field_name)
                        .ok_or_else(|| Diagnostic::new(format!("unknown field `{field_name}`")))?;
                    self.emit_initializer(
                        Storage {
                            address: storage.address + field.offset,
                            size: field.size,
                        },
                        &field.ty,
                        value,
                    )?;
                }
            }
            (Type::Named(name), Expr::Ident(source)) if self.model.structs.contains_key(&name) => {
                let source = self.binding(source)?;
                self.copy(source.memory()?, storage, storage.size);
            }
            (resolved @ (Type::Array { .. } | Type::Named(_)), Expr::Deref(pointer)) => {
                self.emit_expr(pointer, &Type::Ptr(Box::new(resolved)))?;
                self.line("    move.l d0,a0");
                self.copy_from_a0(storage, storage.size);
            }
            (_, value) => {
                self.emit_expr(value, ty)?;
                self.store_d0(storage, ty)?;
            }
        }
        Ok(())
    }

    fn jump_loop(&mut self, continue_loop: bool) -> Result<(), Diagnostic> {
        let labels = self
            .loops
            .last()
            .ok_or_else(|| {
                Diagnostic::new(if continue_loop {
                    "continue outside loop"
                } else {
                    "break outside loop"
                })
            })?
            .clone();
        self.line(&format!(
            "    bra {}",
            if continue_loop {
                labels.continue_label
            } else {
                labels.break_label
            }
        ));
        Ok(())
    }

    fn emit_memory_shift_assignment(
        &mut self,
        target: &Place,
        ty: &Type,
        op: AssignOp,
        count: u32,
    ) -> Result<bool, Diagnostic> {
        let Place::Ident(name) = target else {
            return Ok(false);
        };
        let binding = self.binding(name)?;
        if binding.volatile || scalar_width(ty)? != 2 || !should_use_memory_shift(count) {
            return Ok(false);
        }
        let instruction = match (op, type_is_signed(ty)) {
            (AssignOp::Shl, _) => "lsl",
            (AssignOp::Shr, true) => "asr",
            (AssignOp::Shr, false) => "lsr",
            _ => return Ok(false),
        };
        let operand = self.memory_operand(self.storage_ref(binding.memory()?)?)?;
        self.line(&format!("    {instruction} {operand}"));
        Ok(true)
    }

    fn emit_local_single_bit_compound_assignment(
        &mut self,
        target: &Place,
        ty: &Type,
        op: AssignOp,
        value: &Expr,
    ) -> Result<bool, Diagnostic> {
        let Place::Ident(name) = target else {
            return Ok(false);
        };
        let Some(binding) = self
            .scopes
            .iter()
            .rev()
            .find_map(|scope| scope.get(name))
            .cloned()
        else {
            return Ok(false);
        };
        let ty = self.model.resolved_type(ty)?;
        if !matches!(ty, Type::Named(ref name) if matches!(name.as_str(), "u8" | "u16" | "u24" | "u32"))
        {
            return Ok(false);
        }
        let Ok(mask) = self.model.const_value(value) else {
            return Ok(false);
        };
        let width = scalar_width(&ty)?;
        let width_mask = (1_u64 << (u32::from(width) * 8)) - 1;
        let mask = (mask as u64) & width_mask;
        let bit_mask = match op {
            AssignOp::BitOr | AssignOp::BitXor if mask.is_power_of_two() => mask,
            AssignOp::BitAnd if (!mask & width_mask).is_power_of_two() => !mask & width_mask,
            _ => return Ok(false),
        };
        let bit = bit_mask.trailing_zeros();
        let byte_offset = u32::from(width - 1) - bit / 8;
        let instruction = match op {
            AssignOp::BitOr => "bset",
            AssignOp::BitAnd => "bclr",
            AssignOp::BitXor => "bchg",
            _ => unreachable!(),
        };
        let operand = self.memory_operand(self.storage_ref(Storage {
            address: binding.memory()?.address + byte_offset,
            size: 1,
        })?)?;
        self.line(&format!("    {instruction} #{},{}", bit % 8, operand));
        Ok(true)
    }

    fn emit_jump_if_false(
        &mut self,
        condition: &Expr,
        false_label: &str,
    ) -> Result<bool, Diagnostic> {
        let Expr::Binary { left, op, right } = condition else {
            return Ok(false);
        };
        if !matches!(op, BinaryOp::Eq | BinaryOp::Ne) {
            return Ok(false);
        }
        let Expr::Binary {
            left: source,
            op: BinaryOp::BitAnd,
            right: mask_expr,
        } = left.as_ref()
        else {
            return Ok(false);
        };
        let (Ok(mask), Ok(expected)) = (
            self.model.const_value(mask_expr),
            self.model.const_value(right),
        ) else {
            return Ok(false);
        };
        let source_ty = self.model.resolved_type(&self.expr_type(source)?)?;
        let width = scalar_width(&source_ty)?;
        let width_mask = (1_u64 << (u32::from(width) * 8)) - 1;
        let Ok(mask) = u64::try_from(mask) else {
            return Ok(false);
        };
        let Ok(expected) = u64::try_from(expected) else {
            return Ok(false);
        };
        if mask == 0
            || mask > width_mask
            || expected > width_mask
            || (expected != 0 && expected != mask)
        {
            return Ok(false);
        }

        self.emit_expr(source, &source_ty)?;
        let branch = if (*op == BinaryOp::Eq) == (expected == 0) {
            "bne"
        } else {
            "beq"
        };
        if mask.is_power_of_two() {
            let bit = mask.trailing_zeros();
            self.line(&format!("    btst #{bit},d0"));
        } else {
            let size = size_suffix(&source_ty)?;
            self.line(&format!("    andi.{size} #${mask:X},d0"));
            if expected == mask {
                self.line(&format!("    cmpi.{size} #${mask:X},d0"));
            }
        }
        self.line(&format!("    {branch} {false_label}"));
        Ok(true)
    }

    fn emit_expr(&mut self, expr: &Expr, expected: &Type) -> Result<(), Diagnostic> {
        self.require_scalar(expected)?;
        if self.emit_byte_swap_combine(expr, expected)? {
            return Ok(());
        }
        if self.emit_rotate_combine(expr, expected)? {
            return Ok(());
        }
        match expr {
            Expr::Int(value) | Expr::TypedInt(value, _) => self.load_constant(*value, expected)?,
            Expr::Bool(value) => self.load_constant(i64::from(*value), expected)?,
            Expr::Char(value) => self.load_constant(i64::from(*value), expected)?,
            Expr::String(value) => {
                let storage = self.model.intern_string(value)?;
                self.load_constant(i64::from(storage.address), expected)?;
            }
            Expr::Ident(name) => {
                if let Some(value) = self.model.constants.get(name).copied() {
                    self.load_constant(value, expected)?;
                } else {
                    let binding = self.binding(name)?;
                    let source_ty = binding.ty.clone();
                    self.load_binding_d0(&binding)?;
                    self.convert_d0(&source_ty, expected)?;
                }
            }
            Expr::Field { base, field } => {
                let constant = format!("{base}.{field}");
                if let Some(value) = self.model.constants.get(&constant).copied() {
                    self.load_constant(value, expected)?;
                } else {
                    let binding = self.binding(base)?;
                    let field = self.model.field(&binding.ty, field)?.clone();
                    self.load_d0(
                        Storage {
                            address: binding.memory()?.address + field.offset,
                            size: field.size,
                        },
                        &field.ty,
                    )?;
                    self.convert_d0(&field.ty, expected)?;
                }
            }
            Expr::Index { name, index } => {
                let element = self.emit_named_index_address(name, index)?;
                self.load_from_a0(&element)?;
                self.convert_d0(&element, expected)?;
            }
            Expr::Access(path) => {
                let ty = self.emit_access_address(path)?;
                self.load_from_a0(&ty)?;
                self.convert_d0(&ty, expected)?;
            }
            Expr::AddressOf(name) => {
                if let Some(function_type) = self.function_value_type(name) {
                    let actual = Type::Ptr(Box::new(function_type));
                    if self.model.resolved_type(&actual)? != self.model.resolved_type(expected)? {
                        return Err(Diagnostic::new(format!(
                            "function `{name}` has a different function-pointer type"
                        )));
                    }
                    self.line(&format!("    move.l #{},d0", function_label(name)));
                    self.normalize_d0(expected)?;
                } else {
                    let binding = self.binding(name)?;
                    self.load_storage_address(binding.memory()?, "a0")?;
                    self.line("    move.l a0,d0");
                }
            }
            Expr::AddressOfIndex { name, index } => {
                self.emit_named_index_address(name, index)?;
                self.line("    move.l a0,d0");
            }
            Expr::AddressOfField { base, field } => {
                let binding = self.binding(base)?;
                let field = self.model.field(&binding.ty, field)?;
                self.load_storage_address(
                    Storage {
                        address: binding.memory()?.address + field.offset,
                        size: field.size,
                    },
                    "a0",
                )?;
                self.line("    move.l a0,d0");
            }
            Expr::AddressOfAccess(path) => {
                self.emit_access_address(path)?;
                self.line("    move.l a0,d0");
            }
            Expr::Call { path, args } => self.emit_call(path, args, Some(expected))?,
            Expr::Unary { op, expr } => {
                self.emit_expr(expr, expected)?;
                match op {
                    UnaryOp::Neg => self.line(&format!("    neg.{} d0", size_suffix(expected)?)),
                    UnaryOp::BitNot => self.line(&format!("    not.{} d0", size_suffix(expected)?)),
                    UnaryOp::Not => self.boolean_from_test("beq", expected)?,
                }
            }
            Expr::Binary { left, op, right } => {
                if matches!(op, BinaryOp::And | BinaryOp::Or) {
                    self.emit_logical(left, *op, right)?;
                    return Ok(());
                }
                let operand_ty = if is_comparison(*op) {
                    self.expr_type(left)?
                } else {
                    expected.clone()
                };
                self.emit_expr(left, &operand_ty)?;
                if matches!(op, BinaryOp::Shl | BinaryOp::Shr)
                    && let Some(count) = self.constant_shift_count(right)
                {
                    self.emit_constant_shift(*op, &operand_ty, count)?;
                    return Ok(());
                }
                self.line("    move.l d0,-(sp)");
                self.emit_expr(right, &operand_ty)?;
                self.line(&format!("    move.{} d0,d2", size_suffix(&operand_ty)?));
                if matches!(op, BinaryOp::Add | BinaryOp::Sub)
                    && let Type::Ptr(element) = self.model.resolved_type(&operand_ty)?
                {
                    let size = self.model.type_size(&element)?;
                    if size > 1 {
                        self.line(&format!("    mulu #${size:X},d2"));
                    }
                }
                self.line("    move.l (sp)+,d0");
                self.emit_binary(*op, &operand_ty)?;
            }
            Expr::Cast { ty, expr } => {
                let ty = self.model.resolved_type(ty)?;
                let source_ty = self.model.resolved_type(&self.expr_type(expr)?)?;
                self.require_scalar(&source_ty)?;
                self.require_scalar(&ty)?;
                self.emit_expr(expr, &source_ty)?;
                self.convert_d0(&source_ty, &ty)?;
            }
            Expr::BankedPointer { pointer, .. } => self.emit_expr(pointer, expected)?,
            Expr::Deref(pointer) => {
                let pointer_ty = self.expr_type(pointer)?;
                let Type::Ptr(inner) = self.model.resolved_type(&pointer_ty)? else {
                    return Err(Diagnostic::new("dereference requires pointer"));
                };
                self.require_scalar(&inner)?;
                self.emit_expr(pointer, &pointer_ty)?;
                self.line("    move.l d0,a0");
                self.load_from_a0(&inner)?;
                self.convert_d0(&inner, expected)?;
            }
            Expr::In(port) => {
                return Err(Diagnostic::new(format!(
                    "M68k does not support separate port I/O `{port}`; use mmio instead"
                )));
            }
            Expr::Array(_) | Expr::StructInit { .. } => {
                return Err(Diagnostic::new(
                    "aggregate value requires a storage context",
                ));
            }
        }
        Ok(())
    }

    fn constant_shift_count(&self, expr: &Expr) -> Option<u32> {
        match expr {
            Expr::Int(value) | Expr::TypedInt(value, _) => u32::try_from(*value).ok(),
            Expr::Ident(name) => self
                .model
                .constants
                .get(name)
                .and_then(|value| u32::try_from(*value).ok()),
            Expr::Cast { expr, .. } => self.constant_shift_count(expr),
            _ => None,
        }
    }

    fn emit_rotate_combine(&mut self, expr: &Expr, expected: &Type) -> Result<bool, Diagnostic> {
        let expected = self.model.resolved_type(expected)?;
        let width = scalar_width(&expected)?;
        if !matches!(width, 1 | 2 | 4) {
            return Ok(false);
        }
        let bits = u32::from(width) * 8;
        let Some((source, left_count)) =
            match_rotate(expr, bits, |value| self.constant_shift_count(value))
        else {
            return Ok(false);
        };
        if !self.can_reuse_without_memory_effects(source) {
            return Ok(false);
        }
        let source_ty = self.model.resolved_type(&self.expr_type(source)?)?;
        if scalar_width(&source_ty)? != width {
            return Ok(false);
        }

        self.emit_expr(source, &source_ty)?;
        if width == 4 && left_count == 16 {
            self.line("    swap d0");
        } else {
            let right_count = bits - left_count;
            let (instruction, mut count) = if left_count <= right_count {
                ("rol", left_count)
            } else {
                ("ror", right_count)
            };
            let suffix = size_suffix(&source_ty)?;
            while count > 8 {
                self.line(&format!("    {instruction}.{suffix} #8,d0"));
                count -= 8;
            }
            if count != 0 {
                self.emit_immediate_shift(instruction, suffix, count, "d0");
            }
        }
        self.convert_d0(&source_ty, &expected)?;
        Ok(true)
    }

    fn emit_byte_swap_combine(&mut self, expr: &Expr, expected: &Type) -> Result<bool, Diagnostic> {
        let expected = self.model.resolved_type(expected)?;
        let width = scalar_width(&expected)?;
        if !matches!(width, 2 | 4) {
            return Ok(false);
        }
        let mut terms = Vec::new();
        flatten_bit_or(expr, &mut terms);
        let source = if width == 2 {
            match_u16_byte_swap(&terms, |value| self.constant_shift_count(value))
        } else {
            match_u32_byte_swap(&terms, |value| self.constant_shift_count(value))
        };
        let Some(source) = source else {
            return Ok(false);
        };
        if !self.can_reuse_without_memory_effects(source) {
            return Ok(false);
        }
        let source_ty = self.model.resolved_type(&self.expr_type(source)?)?;
        if scalar_width(&source_ty)? != width {
            return Ok(false);
        }
        self.emit_expr(source, &source_ty)?;
        if width == 2 {
            self.line("    ror.w #8,d0");
        } else {
            self.line("    ror.w #8,d0");
            self.line("    swap d0");
            self.line("    ror.w #8,d0");
        }
        self.convert_d0(&source_ty, &expected)?;
        Ok(true)
    }

    fn can_reuse_without_memory_effects(&self, expr: &Expr) -> bool {
        match expr {
            Expr::Int(_)
            | Expr::TypedInt(_, _)
            | Expr::Bool(_)
            | Expr::Char(_)
            | Expr::String(_)
            | Expr::AddressOf(_)
            | Expr::AddressOfIndex { .. }
            | Expr::AddressOfField { .. }
            | Expr::AddressOfAccess(_) => true,
            Expr::Ident(name) => !self.model.mmio.contains_key(name),
            Expr::Unary { expr, .. }
            | Expr::BankedPointer { pointer: expr, .. }
            | Expr::Cast { expr, .. } => self.can_reuse_without_memory_effects(expr),
            Expr::Binary { left, right, .. } => {
                self.can_reuse_without_memory_effects(left)
                    && self.can_reuse_without_memory_effects(right)
            }
            Expr::Field { .. }
            | Expr::Index { .. }
            | Expr::Access(_)
            | Expr::Deref(_)
            | Expr::Call { .. }
            | Expr::In(_)
            | Expr::Array(_)
            | Expr::StructInit { .. } => false,
        }
    }

    fn emit_constant_shift(
        &mut self,
        op: BinaryOp,
        ty: &Type,
        count: u32,
    ) -> Result<(), Diagnostic> {
        let width = scalar_width(ty)?;
        let bits = u32::from(width) * 8;
        if count == 0 {
            return Ok(());
        }
        if count >= bits {
            if op == BinaryOp::Shr && type_is_signed(ty) {
                self.extend_signed_to_long(ty)?;
                for _ in 0..width {
                    self.line("    asr.l #8,d0");
                }
            } else {
                self.line("    moveq #0,d0");
            }
            return Ok(());
        }
        if count == 16 && matches!(width, 3 | 4) {
            self.emit_word_boundary_shift(op, ty)?;
            return Ok(());
        }
        let instruction = match op {
            BinaryOp::Shl => "lsl",
            BinaryOp::Shr if type_is_signed(ty) => "asr",
            BinaryOp::Shr => "lsr",
            _ => unreachable!("constant shift called for a non-shift operation"),
        };
        let suffix = size_suffix(ty)?;
        let mut remaining = count;
        while remaining > 8 {
            self.line(&format!("    {instruction}.{suffix} #8,d0"));
            remaining -= 8;
        }
        if remaining != 0 {
            self.line(&format!("    {instruction}.{suffix} #{remaining},d0"));
        }
        self.normalize_d0(ty)?;
        Ok(())
    }

    fn emit_word_boundary_shift(&mut self, op: BinaryOp, ty: &Type) -> Result<(), Diagnostic> {
        let width = scalar_width(ty)?;
        match (op, width, type_is_signed(ty)) {
            (BinaryOp::Shl, 4, _) => {
                self.line("    swap d0");
                self.line("    clr.w d0");
            }
            (BinaryOp::Shl, 3, _) => {
                self.line("    swap d0");
                self.line("    andi.l #$FFFF00,d0");
            }
            (BinaryOp::Shr, 4, true) => {
                self.line("    swap d0");
                self.line("    ext.l d0");
            }
            (BinaryOp::Shr, 4, false) => {
                self.line("    swap d0");
                self.line("    andi.l #$FFFF,d0");
            }
            (BinaryOp::Shr, 3, true) => {
                self.line("    swap d0");
                self.line("    ext.w d0");
                self.line("    ext.l d0");
            }
            (BinaryOp::Shr, 3, false) => {
                self.line("    swap d0");
                self.line("    andi.l #$FF,d0");
            }
            _ => unreachable!("word boundary shift called for an unsupported type"),
        }
        self.normalize_d0(ty)
    }

    fn convert_d0(&mut self, source: &Type, target: &Type) -> Result<(), Diagnostic> {
        let source = self.model.resolved_type(source)?;
        let target = self.model.resolved_type(target)?;
        let source_width = scalar_width(&source)?;
        let target_width = scalar_width(&target)?;
        if source_width == target_width {
            return Ok(());
        }
        if source_width > target_width {
            self.line(&format!("    andi.l #${:X},d0", width_mask(target_width)));
            if target_width == 3 {
                self.normalize_d0(&target)?;
            }
            return Ok(());
        }
        if type_is_signed(&source) {
            match source_width {
                1 => {
                    self.line("    ext.w d0");
                    if target_width >= 3 {
                        self.line("    ext.l d0");
                    }
                }
                2 => {
                    if target_width >= 3 {
                        self.line("    ext.l d0");
                    }
                }
                3 => self.normalize_d0(&source)?,
                _ => unreachable!(),
            }
        } else {
            self.line(&format!("    andi.l #${:X},d0", width_mask(source_width)));
        }
        if target_width == 3 {
            self.normalize_d0(&target)?;
        }
        Ok(())
    }

    fn extend_signed_to_long(&mut self, ty: &Type) -> Result<(), Diagnostic> {
        match scalar_width(ty)? {
            1 => {
                self.line("    ext.w d0");
                self.line("    ext.l d0");
            }
            2 => self.line("    ext.l d0"),
            3 => self.normalize_d0(ty)?,
            4 => {}
            _ => unreachable!(),
        }
        Ok(())
    }

    fn emit_binary(&mut self, op: BinaryOp, ty: &Type) -> Result<(), Diagnostic> {
        let size = size_suffix(ty)?;
        if scalar_width(ty)? >= 3 && matches!(op, BinaryOp::Mul | BinaryOp::Div | BinaryOp::Mod) {
            return if op == BinaryOp::Mul {
                self.emit_wide_multiply(ty)
            } else {
                self.emit_wide_divide(op == BinaryOp::Mod, ty)
            };
        }
        match op {
            BinaryOp::Add => self.line(&format!("    add.{size} d2,d0")),
            BinaryOp::Sub => self.line(&format!("    sub.{size} d2,d0")),
            BinaryOp::BitAnd => self.line(&format!("    and.{size} d2,d0")),
            BinaryOp::BitOr => self.line(&format!("    or.{size} d2,d0")),
            BinaryOp::BitXor => self.line(&format!("    eor.{size} d2,d0")),
            BinaryOp::Shl => self.line(&format!("    lsl.{size} d2,d0")),
            BinaryOp::Shr => self.line(&format!(
                "    {}.{size} d2,d0",
                if type_is_signed(ty) { "asr" } else { "lsr" }
            )),
            BinaryOp::Mul => {
                if size == "b" {
                    if type_is_signed(ty) {
                        self.line("    ext.w d0");
                        self.line("    ext.w d2");
                    } else {
                        self.line("    andi.w #$FF,d0");
                        self.line("    andi.w #$FF,d2");
                    }
                }
                self.line(if type_is_signed(ty) {
                    "    muls.w d2,d0"
                } else {
                    "    mulu.w d2,d0"
                });
            }
            BinaryOp::Div | BinaryOp::Mod => {
                if size == "b" {
                    return Err(Diagnostic::new("M68k division requires u16 or i16 values"));
                }
                let zero = self.next_label("div_u16_zero");
                let done = self.next_label("div_u16_done");
                self.line("    tst.w d2");
                self.line(&format!("    beq {zero}"));
                if type_is_signed(ty) {
                    self.line("    ext.l d0");
                    self.line("    divs d2,d0");
                } else {
                    self.line("    andi.l #$FFFF,d0");
                    self.line("    divu d2,d0");
                }
                if op == BinaryOp::Mod {
                    self.line("    swap d0");
                }
                self.line(&format!("    bra {done}"));
                self.line(&format!("{zero}:"));
                self.line("    moveq #0,d0");
                self.line(&format!("{done}:"));
            }
            BinaryOp::Eq
            | BinaryOp::Ne
            | BinaryOp::Lt
            | BinaryOp::Le
            | BinaryOp::Gt
            | BinaryOp::Ge => {
                self.line(&format!("    cmp.{size} d2,d0"));
                self.boolean_from_branch(comparison_branch(op, type_is_signed(ty)))?;
            }
            BinaryOp::And | BinaryOp::Or => unreachable!(),
        }
        if !is_comparison(op) {
            self.normalize_d0(ty)?;
        }
        Ok(())
    }

    fn emit_wide_multiply(&mut self, ty: &Type) -> Result<(), Diagnostic> {
        let signed = type_is_signed(ty);
        let width = scalar_width(ty)? * 8;
        let loop_label = self.next_label(&format!("mul_u{width}_loop"));
        let skip = self.next_label(&format!("mul_u{width}_skip"));
        let done = self.next_label(&format!("mul_u{width}_done"));
        let positive_left = self.next_label(&format!("mul_u{width}_left_positive"));
        let positive_right = self.next_label(&format!("mul_u{width}_right_positive"));
        let positive_result = self.next_label(&format!("mul_u{width}_result_positive"));
        self.line("    moveq #0,d5");
        if signed {
            self.line("    tst.l d0");
            self.line(&format!("    bpl {positive_left}"));
            self.line("    neg.l d0");
            self.line("    moveq #1,d5");
            self.line(&format!("{positive_left}:"));
            self.line("    tst.l d2");
            self.line(&format!("    bpl {positive_right}"));
            self.line("    neg.l d2");
            self.line("    eori.b #1,d5");
            self.line(&format!("{positive_right}:"));
        }
        self.line("    move.l d0,d3");
        self.line("    move.l d2,d4");
        self.line("    moveq #0,d1");
        self.line(&format!("{loop_label}:"));
        self.line("    tst.l d4");
        self.line(&format!("    beq {done}"));
        self.line("    move.l d4,d6");
        self.line("    andi.l #1,d6");
        self.line(&format!("    beq {skip}"));
        self.line("    add.l d3,d1");
        self.line(&format!("{skip}:"));
        self.line("    lsl.l #1,d3");
        self.line("    lsr.l #1,d4");
        self.line(&format!("    bra {loop_label}"));
        self.line(&format!("{done}:"));
        self.line("    move.l d1,d0");
        if signed {
            self.line("    tst.b d5");
            self.line(&format!("    beq {positive_result}"));
            self.line("    neg.l d0");
            self.line(&format!("{positive_result}:"));
        }
        self.normalize_d0(ty)
    }

    fn emit_wide_divide(&mut self, remainder: bool, ty: &Type) -> Result<(), Diagnostic> {
        let signed = type_is_signed(ty);
        let width = scalar_width(ty)? * 8;
        let divisor_zero = self.next_label(&format!("div_u{width}_zero"));
        let loop_label = self.next_label(&format!("div_u{width}_loop"));
        let result_ready = self.next_label(&format!("div_u{width}_result_ready"));
        let done = self.next_label(&format!("div_u{width}_done"));
        let positive_left = self.next_label(&format!("div_u{width}_left_positive"));
        let positive_right = self.next_label(&format!("div_u{width}_right_positive"));
        let positive_result = self.next_label(&format!("div_u{width}_result_positive"));
        self.line("    moveq #0,d5");
        if signed {
            self.line("    tst.l d0");
            self.line(&format!("    bpl {positive_left}"));
            self.line("    neg.l d0");
            // Bit 0 is the quotient sign; bit 1 is the dividend/remainder sign.
            self.line("    moveq #3,d5");
            self.line(&format!("{positive_left}:"));
            self.line("    tst.l d2");
            self.line(&format!("    bpl {positive_right}"));
            self.line("    neg.l d2");
            self.line("    eori.b #1,d5");
            self.line(&format!("{positive_right}:"));
        }
        self.line("    tst.l d2");
        self.line(&format!("    beq {divisor_zero}"));
        self.line("    moveq #0,d1");
        self.line(&format!("    moveq #{width},d4"));
        self.line(&format!("{loop_label}:"));
        self.line("    move.l d0,d6");
        self.line("    rol.l #1,d6");
        self.line("    andi.l #1,d6");
        self.line("    move.l d1,d3");
        self.line("    rol.l #1,d3");
        self.line("    andi.l #1,d3");
        self.line("    add.l d0,d0");
        self.line("    add.l d1,d1");
        self.line("    add.l d6,d1");
        self.line("    move.l d1,d6");
        self.line("    eori.l #$80000000,d6");
        self.line("    move.l d2,d7");
        self.line("    eori.l #$80000000,d7");
        self.line("    cmp.l d7,d6");
        self.line("    sge d6");
        self.line("    andi.l #1,d6");
        self.line("    or.l d3,d6");
        self.line("    neg.l d6");
        self.line("    move.l d2,d3");
        self.line("    and.l d6,d3");
        self.line("    sub.l d3,d1");
        self.line("    andi.l #1,d6");
        self.line("    add.l d6,d0");
        self.line("    subq.w #1,d4");
        self.line(&format!("    bne {loop_label}"));
        if remainder {
            self.line("    move.l d1,d0");
        }
        self.line(&format!("    bra {result_ready}"));
        self.line(&format!("{divisor_zero}:"));
        self.line("    moveq #0,d0");
        self.line(&format!("    bra {done}"));
        self.line(&format!("{result_ready}:"));
        if signed {
            self.line(&format!("    btst #{},d5", if remainder { 1 } else { 0 }));
            self.line(&format!("    beq {positive_result}"));
            self.line("    neg.l d0");
            self.line(&format!("{positive_result}:"));
        }
        self.line(&format!("{done}:"));
        self.normalize_d0(ty)
    }

    fn emit_logical(&mut self, left: &Expr, op: BinaryOp, right: &Expr) -> Result<(), Diagnostic> {
        let decisive = self.next_label("logical_decisive");
        let done = self.next_label("logical_done");
        let bool_ty = Type::Named("bool".to_owned());
        self.emit_expr(left, &bool_ty)?;
        self.line("    tst.b d0");
        self.line(&format!(
            "    {} {decisive}",
            if op == BinaryOp::And { "beq" } else { "bne" }
        ));
        self.emit_expr(right, &bool_ty)?;
        self.line(&format!("    bra {done}"));
        self.line(&format!("{decisive}:"));
        self.line(&format!(
            "    moveq #{},d0",
            if op == BinaryOp::And { 0 } else { 1 }
        ));
        self.line(&format!("{done}:"));
        Ok(())
    }

    fn emit_intrinsic(
        &mut self,
        operation: IntrinsicOperation,
        args: &[Expr],
        expected: Option<&Type>,
    ) -> Result<(), Diagnostic> {
        let result = intrinsic_result_type(operation, args, |arg| self.expr_type(arg))?;
        if !result.is_empty() {
            if let Some(expected) = expected {
                let expected = self.model.resolved_type(expected)?;
                let actual = self.model.resolved_type(&result[0])?;
                if expected != actual {
                    return Err(Diagnostic::new(format!(
                        "intrinsic result has type {actual:?}, expected {expected:?}"
                    )));
                }
            }
        }
        match operation {
            IntrinsicOperation::Bits(operation) => self.emit_bits_intrinsic(operation, args),
            IntrinsicOperation::Int(operation) => self.emit_int_intrinsic(operation, args),
            IntrinsicOperation::Mem(operation) => self.emit_mem_intrinsic(operation, args),
        }
    }

    fn emit_bits_intrinsic(
        &mut self,
        operation: BitsIntrinsic,
        args: &[Expr],
    ) -> Result<(), Diagnostic> {
        match operation {
            BitsIntrinsic::RotateLeft | BitsIntrinsic::RotateRight => {
                self.require_arg_count(args, 2, "rotate")?;
                let ty = self.model.resolved_type(&self.expr_type(&args[0])?)?;
                let width = require_unsigned_bit_width(&ty)?;
                self.emit_expr(&args[0], &ty)?;
                self.line("    move.l d0,-(sp)");
                self.emit_expr(&args[1], &Type::Named("u24".to_owned()))?;
                self.line("    move.l d0,d2");
                self.line("    move.l (sp)+,d0");
                let bits = u32::from(width) * 8;
                if width == 3 {
                    self.line("    andi.l #$FFFFFF,d0");
                    self.line(&format!("    andi.l #${:X},d2", bits - 1));
                    let loop_label = self.next_label("intrinsic_rotate_loop");
                    let done = self.next_label("intrinsic_rotate_done");
                    self.line(&format!("{loop_label}:"));
                    self.line(&format!("    tst.l d2"));
                    self.line(&format!("    beq {done}"));
                    if operation == BitsIntrinsic::RotateLeft {
                        self.line("    move.l d0,d1");
                        self.line("    andi.l #$800000,d1");
                        self.line("    lsl.l #1,d0");
                        self.line("    andi.l #$FFFFFF,d0");
                        self.line("    tst.l d1");
                        let no_wrap = self.next_label("intrinsic_rotate_no_wrap");
                        self.line(&format!("    beq {no_wrap}"));
                        self.line("    ori.l #1,d0");
                        self.line(&format!("{no_wrap}:"));
                    } else {
                        self.line("    move.l d0,d1");
                        self.line("    andi.l #1,d1");
                        self.line("    lsr.l #1,d0");
                        self.line("    tst.l d1");
                        let no_wrap = self.next_label("intrinsic_rotate_no_wrap");
                        self.line(&format!("    beq {no_wrap}"));
                        self.line("    ori.l #$800000,d0");
                        self.line(&format!("{no_wrap}:"));
                    }
                    self.line("    subq.l #1,d2");
                    self.line(&format!("    bra {loop_label}"));
                    self.line(&format!("{done}:"));
                } else {
                    self.line(&format!("    andi.w #${:X},d2", bits - 1));
                    let suffix = if width == 1 { "b" } else { "w" };
                    let instruction = if operation == BitsIntrinsic::RotateLeft {
                        "rol"
                    } else {
                        "ror"
                    };
                    self.line(&format!("    {instruction}.{suffix} d2,d0"));
                }
                self.mask_d0(width);
            }
            BitsIntrinsic::Test
            | BitsIntrinsic::Set
            | BitsIntrinsic::Clear
            | BitsIntrinsic::Toggle => {
                self.require_arg_count(args, 2, "bit intrinsic")?;
                let ty = self.model.resolved_type(&self.expr_type(&args[0])?)?;
                let width = require_unsigned_bit_width(&ty)?;
                let bit = self.intrinsic_constant(args, 1, "bit index")?;
                let bit = u32::try_from(bit)
                    .ok()
                    .filter(|bit| *bit < u32::from(width) * 8)
                    .ok_or_else(|| Diagnostic::new("bit index must be within the input width"))?;
                self.emit_expr(&args[0], &ty)?;
                match operation {
                    BitsIntrinsic::Test => {
                        self.line(&format!("    btst #{bit},d0"));
                        self.boolean_from_branch("bne")?;
                    }
                    BitsIntrinsic::Set => self.line(&format!("    bset #{bit},d0")),
                    BitsIntrinsic::Clear => self.line(&format!("    bclr #{bit},d0")),
                    BitsIntrinsic::Toggle => self.line(&format!("    bchg #{bit},d0")),
                    _ => unreachable!(),
                }
                self.mask_d0(width);
            }
            BitsIntrinsic::Extract => self.emit_extract_intrinsic(args)?,
            BitsIntrinsic::Insert => self.emit_insert_intrinsic(args)?,
            BitsIntrinsic::ByteSwap => self.emit_byte_swap_intrinsic(args)?,
            BitsIntrinsic::Reverse => self.emit_reverse_intrinsic(args)?,
            BitsIntrinsic::CountOnes
            | BitsIntrinsic::LeadingZeros
            | BitsIntrinsic::TrailingZeros => self.emit_bit_count_intrinsic(operation, args)?,
        }
        Ok(())
    }

    fn emit_extract_intrinsic(&mut self, args: &[Expr]) -> Result<(), Diagnostic> {
        self.require_arg_count(args, 3, "bits.extract")?;
        let ty = self.model.resolved_type(&self.expr_type(&args[0])?)?;
        let width = require_unsigned_bit_width(&ty)?;
        let offset = self.intrinsic_constant(args, 1, "bit-range offset")?;
        let field_width = self.intrinsic_constant(args, 2, "bit-range width")?;
        let (offset, field_width) = self.validate_bit_range(offset, field_width, width)?;
        self.emit_expr(&args[0], &ty)?;
        self.mask_d0(width);
        let suffix = if width == 1 { "b" } else { "l" };
        if offset != 0 {
            self.emit_immediate_shift("lsr", suffix, offset, "d0");
        }
        self.line(&format!("    andi.l #${:X},d0", (1_u32 << field_width) - 1));
        self.mask_d0(width);
        Ok(())
    }

    fn emit_insert_intrinsic(&mut self, args: &[Expr]) -> Result<(), Diagnostic> {
        self.require_arg_count(args, 4, "bits.insert")?;
        let base_ty = self.model.resolved_type(&self.expr_type(&args[0])?)?;
        let value_ty = self.model.resolved_type(&self.expr_type(&args[1])?)?;
        if base_ty != value_ty {
            return Err(Diagnostic::new(
                "bits.insert operands must have the same type",
            ));
        }
        let width = require_unsigned_bit_width(&base_ty)?;
        let offset = self.intrinsic_constant(args, 2, "bit-range offset")?;
        let field_width = self.intrinsic_constant(args, 3, "bit-range width")?;
        let (offset, field_width) = self.validate_bit_range(offset, field_width, width)?;
        self.emit_expr(&args[0], &base_ty)?;
        self.line("    move.l d0,-(sp)");
        self.emit_expr(&args[1], &value_ty)?;
        self.mask_d0(width);
        self.line(&format!("    andi.l #${:X},d0", (1_u32 << field_width) - 1));
        if offset != 0 {
            self.emit_immediate_shift("lsl", "l", offset, "d0");
        }
        self.line("    move.l d0,d1");
        self.line("    move.l (sp)+,d0");
        let field_mask = ((1_u32 << field_width) - 1) << offset;
        self.line(&format!(
            "    andi.l #${:X},d0",
            (!field_mask) & width_mask(width)
        ));
        self.line("    or.l d1,d0");
        self.mask_d0(width);
        Ok(())
    }

    fn emit_byte_swap_intrinsic(&mut self, args: &[Expr]) -> Result<(), Diagnostic> {
        self.require_arg_count(args, 1, "bits.byte_swap")?;
        let ty = self.model.resolved_type(&self.expr_type(&args[0])?)?;
        let width = scalar_width(&ty)?;
        if !matches!(width, 2 | 3)
            || !matches!(ty, Type::Named(ref name) if name == "u16" || name == "u24")
        {
            return Err(Diagnostic::new("bits.byte_swap requires u16 or u24"));
        }
        self.emit_expr(&args[0], &ty)?;
        if width == 2 {
            self.line("    ror.w #8,d0");
        } else {
            self.line("    move.l d0,d1");
            self.line("    move.l d0,d2");
            self.line("    andi.l #$FF,d0");
            self.emit_immediate_shift("lsl", "l", 16, "d0");
            self.line("    andi.l #$FF0000,d1");
            self.emit_immediate_shift("lsr", "l", 16, "d1");
            self.line("    andi.l #$FF00,d2");
            self.line("    or.l d1,d0");
            self.line("    or.l d2,d0");
        }
        self.mask_d0(width);
        Ok(())
    }

    fn emit_reverse_intrinsic(&mut self, args: &[Expr]) -> Result<(), Diagnostic> {
        self.require_arg_count(args, 1, "bits.reverse")?;
        let ty = self.model.resolved_type(&self.expr_type(&args[0])?)?;
        let width = require_unsigned_bit_width(&ty)?;
        self.emit_expr(&args[0], &ty)?;
        self.mask_d0(width);
        self.line("    move.l d0,d1");
        self.line("    moveq #0,d2");
        self.line(&format!("    moveq #{},d3", u32::from(width) * 8));
        let loop_label = self.next_label("intrinsic_reverse_loop");
        let done = self.next_label("intrinsic_reverse_done");
        self.line(&format!("{loop_label}:"));
        self.line("    lsr.l #1,d1");
        self.line("    roxl.l #1,d2");
        self.line("    subq.l #1,d3");
        self.line(&format!("    bne {loop_label}"));
        self.line(&format!("{done}:"));
        self.line("    move.l d2,d0");
        self.mask_d0(width);
        Ok(())
    }

    fn emit_bit_count_intrinsic(
        &mut self,
        operation: BitsIntrinsic,
        args: &[Expr],
    ) -> Result<(), Diagnostic> {
        self.require_arg_count(args, 1, "bit count")?;
        let ty = self.model.resolved_type(&self.expr_type(&args[0])?)?;
        let width = require_unsigned_bit_width(&ty)?;
        self.emit_expr(&args[0], &ty)?;
        self.mask_d0(width);
        let bits = u32::from(width) * 8;
        self.line("    moveq #0,d1");
        match operation {
            BitsIntrinsic::CountOnes => {
                self.line(&format!("    moveq #{bits},d2"));
                let loop_label = self.next_label("intrinsic_ones_loop");
                let skip = self.next_label("intrinsic_ones_skip");
                self.line(&format!("{loop_label}:"));
                self.line("    lsr.l #1,d0");
                self.line(&format!("    bcc {skip}"));
                self.line("    addq.l #1,d1");
                self.line(&format!("{skip}:"));
                self.line("    subq.l #1,d2");
                self.line(&format!("    bne {loop_label}"));
            }
            BitsIntrinsic::LeadingZeros => {
                self.line(&format!("    moveq #{bits},d2"));
                let loop_label = self.next_label("intrinsic_lz_loop");
                let found = self.next_label("intrinsic_lz_found");
                let done = self.next_label("intrinsic_lz_done");
                self.line(&format!("{loop_label}:"));
                self.line("    lsl.l #1,d0");
                self.line(&format!("    bcs {found}"));
                self.line("    addq.l #1,d1");
                self.line("    subq.l #1,d2");
                self.line(&format!("    bne {loop_label}"));
                self.line(&format!("    bra {done}"));
                self.line(&format!("{found}:"));
                self.line(&format!("{done}:"));
            }
            BitsIntrinsic::TrailingZeros => {
                self.line(&format!("    moveq #{bits},d2"));
                let loop_label = self.next_label("intrinsic_tz_loop");
                let found = self.next_label("intrinsic_tz_found");
                self.line(&format!("{loop_label}:"));
                self.line("    lsr.l #1,d0");
                self.line(&format!("    bcs {found}"));
                self.line("    addq.l #1,d1");
                self.line("    subq.l #1,d2");
                self.line(&format!("    bne {loop_label}"));
                self.line(&format!("{found}:"));
            }
            _ => unreachable!(),
        }
        self.line("    move.l d1,d0");
        self.mask_d0(1);
        Ok(())
    }

    fn emit_int_intrinsic(
        &mut self,
        operation: IntIntrinsic,
        args: &[Expr],
    ) -> Result<(), Diagnostic> {
        match operation {
            IntIntrinsic::WideningMul => self.emit_widening_mul_intrinsic(args),
            IntIntrinsic::MulHigh => {
                self.emit_full_mul_intrinsic(args)?;
                let ty = self.model.resolved_type(&self.expr_type(&args[0])?)?;
                self.line("    move.l d1,d0");
                self.mask_d0(scalar_width(&ty)?);
                Ok(())
            }
            IntIntrinsic::SaturatingAdd | IntIntrinsic::SaturatingSub => {
                self.emit_saturating_intrinsic(operation, args)
            }
            IntIntrinsic::Divmod
            | IntIntrinsic::AddCarry
            | IntIntrinsic::SubBorrow
            | IntIntrinsic::FullMul => Err(Diagnostic::new(
                "two-result integer intrinsic requires a two-result binding",
            )),
        }
    }

    fn emit_widening_mul_intrinsic(&mut self, args: &[Expr]) -> Result<(), Diagnostic> {
        self.require_arg_count(args, 2, "int.widening_mul")?;
        let left_ty = self.model.resolved_type(&self.expr_type(&args[0])?)?;
        let right_ty = self.model.resolved_type(&self.expr_type(&args[1])?)?;
        let result_ty = intrinsic_result_type(
            IntrinsicOperation::Int(IntIntrinsic::WideningMul),
            args,
            |arg| self.expr_type(arg),
        )?
        .into_iter()
        .next()
        .ok_or_else(|| Diagnostic::new("missing widening multiply result type"))?;
        self.emit_expr(&args[0], &left_ty)?;
        self.line("    move.l d0,-(sp)");
        self.emit_expr(&args[1], &right_ty)?;
        self.line("    move.l d0,d2");
        self.line("    move.l (sp)+,d0");
        let left_width = scalar_width(&left_ty)?;
        let right_width = scalar_width(&right_ty)?;
        if left_width <= 2 && right_width <= 2 {
            if type_is_signed(&left_ty) {
                self.line("    ext.w d0");
                self.line("    ext.w d2");
                self.line("    muls.w d2,d0");
            } else {
                self.mask_register("d0", left_width);
                self.mask_register("d2", right_width);
                self.line("    mulu.w d2,d0");
            }
            self.mask_d0(scalar_width(&result_ty)?);
        } else {
            if type_is_signed(&left_ty) {
                self.extend_signed_to_long(&left_ty)?;
                self.extend_register_signed("d2", &right_ty)?;
            } else {
                self.mask_register("d0", left_width);
                self.mask_register("d2", right_width);
            }
            self.emit_wide_multiply(&result_ty)?;
        }
        Ok(())
    }

    fn emit_full_mul_intrinsic(&mut self, args: &[Expr]) -> Result<(), Diagnostic> {
        self.require_arg_count(args, 2, "int.full_mul")?;
        let ty = self.model.resolved_type(&self.expr_type(&args[0])?)?;
        let other = self.model.resolved_type(&self.expr_type(&args[1])?)?;
        if ty != other {
            return Err(Diagnostic::new(
                "int.full_mul operands must have the same type",
            ));
        }
        let width = scalar_width(&ty)?;
        self.emit_expr(&args[0], &ty)?;
        self.line("    move.l d0,-(sp)");
        self.emit_expr(&args[1], &ty)?;
        self.line("    move.l d0,d2");
        self.line("    move.l (sp)+,d0");
        if width == 3 {
            self.emit_full_mul_24(&ty)?;
            return Ok(());
        }
        if width > 2 {
            return Err(Diagnostic::new(
                "M68k full_mul and mul_high support at most 24-bit operands",
            ));
        }
        if type_is_signed(&ty) {
            self.line("    ext.w d0");
            self.line("    ext.w d2");
            self.line("    muls.w d2,d0");
        } else {
            self.mask_d0(width);
            self.mask_register("d2", width);
            self.line("    mulu.w d2,d0");
        }
        let shift = u32::from(width) * 8;
        self.line(&format!("    move.l d0,d1"));
        if type_is_signed(&ty) {
            self.emit_immediate_shift("asr", "l", shift, "d1");
        } else {
            self.emit_immediate_shift("lsr", "l", shift, "d1");
        }
        self.mask_d0(width);
        self.mask_register("d1", width);
        Ok(())
    }

    fn emit_full_mul_24(&mut self, ty: &Type) -> Result<(), Diagnostic> {
        let scratch = (0..12)
            .map(|offset| self.scratch_operand(offset))
            .collect::<Result<Vec<_>, _>>()?;
        self.mask_d0(3);
        self.mask_register("d2", 3);
        self.line(&format!("    move.b d0,{}", scratch[2]));
        self.line("    move.l d0,d4");
        self.line("    lsr.l #8,d4");
        self.line(&format!("    move.b d4,{}", scratch[1]));
        self.line("    lsr.l #8,d4");
        self.line(&format!("    move.b d4,{}", scratch[0]));
        self.line(&format!("    move.b d2,{}", scratch[5]));
        self.line("    move.l d2,d4");
        self.line("    lsr.l #8,d4");
        self.line(&format!("    move.b d4,{}", scratch[4]));
        self.line("    lsr.l #8,d4");
        self.line(&format!("    move.b d4,{}", scratch[3]));
        for offset in 0..6 {
            self.line(&format!("    clr.b {}", scratch[6 + offset]));
        }
        self.line("    moveq #0,d6");
        let sign = self.next_label("intrinsic_mul24_positive");
        if type_is_signed(ty) {
            self.line("    moveq #0,d5");
            self.line("    tst.l d0");
            self.line(&format!("    bpl {sign}"));
            self.line("    neg.l d0");
            self.line("    andi.l #$FFFFFF,d0");
            self.line("    moveq #1,d5");
            self.line(&format!("{sign}:"));
            let right_positive = self.next_label("intrinsic_mul24_right_positive");
            self.line("    tst.l d2");
            self.line(&format!("    bpl {right_positive}"));
            self.line("    neg.l d2");
            self.line("    andi.l #$FFFFFF,d2");
            self.line("    eori.b #1,d5");
            self.line(&format!("{right_positive}:"));
            self.line(&format!("    move.b d0,{}", scratch[2]));
            self.line("    move.l d0,d4");
            self.line("    lsr.l #8,d4");
            self.line(&format!("    move.b d4,{}", scratch[1]));
            self.line("    lsr.l #8,d4");
            self.line(&format!("    move.b d4,{}", scratch[0]));
            self.line(&format!("    move.b d2,{}", scratch[5]));
            self.line("    move.l d2,d4");
            self.line("    lsr.l #8,d4");
            self.line(&format!("    move.b d4,{}", scratch[4]));
            self.line("    lsr.l #8,d4");
            self.line(&format!("    move.b d4,{}", scratch[3]));
        }
        self.line("    moveq #24,d3");
        let loop_label = self.next_label("intrinsic_mul24_loop");
        let skip = self.next_label("intrinsic_mul24_skip");
        let done = self.next_label("intrinsic_mul24_done");
        self.line(&format!("{loop_label}:"));
        self.line(&format!("    btst #0,{}", scratch[3]));
        self.line(&format!("    beq {skip}"));
        self.line(&format!("    move.b {},d4", scratch[6]));
        self.line(&format!("    add.b {},d4", scratch[0]));
        self.line(&format!("    move.b d4,{}", scratch[6]));
        for offset in 1..3 {
            self.line(&format!("    move.b {},d4", scratch[6 + offset]));
            self.line(&format!("    addx.b {},d4", scratch[offset]));
            self.line(&format!("    move.b d4,{}", scratch[6 + offset]));
        }
        for offset in 3..6 {
            self.line(&format!("    move.b {},d4", scratch[6 + offset]));
            self.line("    addx.b d6,d4");
            self.line(&format!("    move.b d4,{}", scratch[6 + offset]));
        }
        self.line(&format!("{skip}:"));
        self.line(&format!("    move.b {},d4", scratch[0]));
        self.line("    lsl.b #1,d4");
        self.line(&format!("    move.b d4,{}", scratch[0]));
        for offset in 1..3 {
            self.line(&format!("    move.b {},d4", scratch[offset]));
            self.line("    roxl.b #1,d4");
            self.line(&format!("    move.b d4,{}", scratch[offset]));
        }
        self.line(&format!("    move.b {},d4", scratch[5]));
        self.line("    lsr.b #1,d4");
        self.line(&format!("    move.b d4,{}", scratch[5]));
        for offset in (3..5).rev() {
            self.line(&format!("    move.b {},d4", scratch[offset]));
            self.line("    roxr.b #1,d4");
            self.line(&format!("    move.b d4,{}", scratch[offset]));
        }
        self.line("    subq.l #1,d3");
        self.line(&format!("    bne {loop_label}"));
        self.line(&format!("{done}:"));
        if type_is_signed(ty) {
            let positive = self.next_label("intrinsic_mul24_result_positive");
            self.line("    tst.b d5");
            self.line(&format!("    beq {positive}"));
            for offset in 0..6 {
                self.line(&format!("    not.b {}", scratch[6 + offset]));
            }
            self.line(&format!("    move.b {},d4", scratch[6]));
            self.line("    addq.b #1,d4");
            self.line(&format!("    move.b d4,{}", scratch[6]));
            self.line("    moveq #0,d6");
            for offset in 1..6 {
                self.line(&format!("    move.b {},d4", scratch[6 + offset]));
                self.line("    addx.b d6,d4");
                self.line(&format!("    move.b d4,{}", scratch[6 + offset]));
            }
            self.line(&format!("{positive}:"));
        }
        self.load_little_24_operands(&scratch[8], &scratch[7], &scratch[6], "d0");
        self.load_little_24_operands(&scratch[11], &scratch[10], &scratch[9], "d1");
        Ok(())
    }

    fn emit_saturating_intrinsic(
        &mut self,
        operation: IntIntrinsic,
        args: &[Expr],
    ) -> Result<(), Diagnostic> {
        self.require_arg_count(args, 2, "saturating arithmetic")?;
        let ty = self.model.resolved_type(&self.expr_type(&args[0])?)?;
        let other = self.model.resolved_type(&self.expr_type(&args[1])?)?;
        if ty != other
            || !matches!(ty, Type::Named(ref name) if name.starts_with('u') || name.starts_with('i'))
        {
            return Err(Diagnostic::new(
                "saturating operands must have the same integer type",
            ));
        }
        let width = scalar_width(&ty)?;
        let suffix = size_suffix(&ty)?;
        self.emit_expr(&args[0], &ty)?;
        self.line("    move.l d0,d3");
        self.line("    move.l d0,-(sp)");
        self.emit_expr(&args[1], &ty)?;
        self.line("    move.l d0,d2");
        self.line("    move.l (sp)+,d0");
        if width == 3 {
            return self.emit_saturating_24(operation, &ty);
        }
        if operation == IntIntrinsic::SaturatingAdd {
            self.line(&format!("    add.{suffix} d2,d0"));
        } else {
            self.line(&format!("    sub.{suffix} d2,d0"));
        }
        let done = self.next_label("intrinsic_sat_done");
        if type_is_signed(&ty) {
            let negative = self.next_label("intrinsic_sat_negative");
            self.line(&format!("    bvc {done}"));
            self.line(&format!("    tst.{suffix} d3"));
            self.line(&format!("    bmi {negative}"));
            self.load_constant((1_i64 << (u32::from(width) * 8 - 1)) - 1, &ty)?;
            self.line(&format!("    bra {done}"));
            self.line(&format!("{negative}:"));
            self.load_constant(1_i64 << (u32::from(width) * 8 - 1), &ty)?;
        } else if operation == IntIntrinsic::SaturatingAdd {
            let overflow = self.next_label("intrinsic_sat_overflow");
            self.line(&format!("    bcs {overflow}"));
            self.line(&format!("    bra {done}"));
            self.line(&format!("{overflow}:"));
            self.load_constant(i64::from(width_mask(width)), &ty)?;
        } else {
            let underflow = self.next_label("intrinsic_sat_underflow");
            self.line(&format!("    bcc {done}"));
            self.line(&format!("{underflow}:"));
            self.line("    moveq #0,d0");
        }
        self.line(&format!("{done}:"));
        self.mask_d0(width);
        Ok(())
    }

    fn emit_saturating_24(&mut self, operation: IntIntrinsic, ty: &Type) -> Result<(), Diagnostic> {
        let done = self.next_label("intrinsic_sat24_done");
        let clamp_min = self.next_label("intrinsic_sat24_min");
        let clamp_max = self.next_label("intrinsic_sat24_max");
        if operation == IntIntrinsic::SaturatingAdd {
            self.line("    add.l d2,d0");
        } else {
            self.line("    sub.l d2,d0");
        }
        if !type_is_signed(ty) {
            self.line("    move.l d0,d4");
            self.line("    andi.l #$FF000000,d4");
            if operation == IntIntrinsic::SaturatingAdd {
                self.line(&format!("    beq {done}"));
                self.load_constant(0xFF_FFFF, ty)?;
            } else {
                self.line(&format!("    beq {done}"));
                self.moveq_zero();
            }
            self.line(&format!("    bra {done}"));
            self.line(&format!("{done}:"));
            self.mask_d0(3);
            return Ok(());
        }
        self.line("    andi.l #$FFFFFF,d0");
        if operation == IntIntrinsic::SaturatingAdd {
            let left_negative = self.next_label("intrinsic_sat24_left_negative");
            self.line("    btst #23,d3");
            self.line(&format!("    bne {left_negative}"));
            self.line("    btst #23,d2");
            self.line(&format!("    bne {done}"));
            self.line("    btst #23,d0");
            self.line(&format!("    beq {done}"));
            self.line(&format!("    bra {clamp_max}"));
            self.line(&format!("{left_negative}:"));
            self.line("    btst #23,d2");
            self.line(&format!("    beq {done}"));
            self.line("    btst #23,d0");
            self.line(&format!("    bne {done}"));
            self.line(&format!("    bra {clamp_min}"));
        } else {
            let left_negative = self.next_label("intrinsic_sat24_left_negative");
            self.line("    btst #23,d3");
            self.line(&format!("    bne {left_negative}"));
            self.line("    btst #23,d2");
            self.line(&format!("    beq {done}"));
            self.line("    btst #23,d0");
            self.line(&format!("    beq {done}"));
            self.line(&format!("    bra {clamp_max}"));
            self.line(&format!("{left_negative}:"));
            self.line("    btst #23,d2");
            self.line(&format!("    bne {done}"));
            self.line("    btst #23,d0");
            self.line(&format!("    bne {done}"));
            self.line(&format!("    bra {clamp_min}"));
        }
        self.line(&format!("{clamp_max}:"));
        self.load_constant(0x7F_FFFF, ty)?;
        self.line(&format!("    bra {done}"));
        self.line(&format!("{clamp_min}:"));
        self.load_constant(0x80_0000, ty)?;
        self.line(&format!("{done}:"));
        self.mask_d0(3);
        Ok(())
    }

    fn moveq_zero(&mut self) {
        self.line("    moveq #0,d0");
    }

    fn emit_mem_intrinsic(
        &mut self,
        operation: MemIntrinsic,
        args: &[Expr],
    ) -> Result<(), Diagnostic> {
        match operation {
            MemIntrinsic::Peek8 => self.emit_peek8(args),
            MemIntrinsic::Poke8 => self.emit_poke8(args),
            MemIntrinsic::CopyNonoverlapping => self.emit_copy_intrinsic(args, false),
            MemIntrinsic::Move => self.emit_copy_intrinsic(args, true),
            MemIntrinsic::Fill => self.emit_fill_intrinsic(args),
            MemIntrinsic::FindByte => self.emit_find_byte_intrinsic(args),
            MemIntrinsic::Compare => self.emit_compare_intrinsic(args),
            MemIntrinsic::LoadLe16 => self.emit_endian_load(args, false, 2),
            MemIntrinsic::LoadLe24 => self.emit_endian_load(args, false, 3),
            MemIntrinsic::LoadBe16 => self.emit_endian_load(args, true, 2),
            MemIntrinsic::LoadBe24 => self.emit_endian_load(args, true, 3),
            MemIntrinsic::StoreLe16 => self.emit_endian_store(args, false, 2),
            MemIntrinsic::StoreLe24 => self.emit_endian_store(args, false, 3),
            MemIntrinsic::StoreBe16 => self.emit_endian_store(args, true, 2),
            MemIntrinsic::StoreBe24 => self.emit_endian_store(args, true, 3),
        }
    }

    fn emit_peek8(&mut self, args: &[Expr]) -> Result<(), Diagnostic> {
        self.require_arg_count(args, 1, "mem.peek8")?;
        self.require_u8_pointer(&args[0])?;
        self.emit_expr(&args[0], &Type::Ptr(Box::new(Type::Named("u8".to_owned()))))?;
        self.line("    move.l d0,a0");
        self.line("    move.b (a0),d0");
        Ok(())
    }

    fn emit_poke8(&mut self, args: &[Expr]) -> Result<(), Diagnostic> {
        self.require_arg_count(args, 2, "mem.poke8")?;
        self.require_u8_pointer(&args[0])?;
        self.emit_expr(&args[0], &Type::Ptr(Box::new(Type::Named("u8".to_owned()))))?;
        self.line("    move.l d0,a0");
        self.emit_expr(&args[1], &Type::Named("u8".to_owned()))?;
        self.line("    move.b d0,(a0)");
        self.line("    moveq #0,d0");
        Ok(())
    }

    fn emit_copy_intrinsic(&mut self, args: &[Expr], overlap_safe: bool) -> Result<(), Diagnostic> {
        self.require_arg_count(args, 3, "memory copy")?;
        self.reject_known_volatile_memory(args, &[0, 1])?;
        self.require_u8_pointer(&args[0])?;
        self.require_u8_pointer(&args[1])?;
        let pointer = Type::Ptr(Box::new(Type::Named("u8".to_owned())));
        self.emit_expr(&args[0], &pointer)?;
        self.line("    move.l d0,a0");
        self.emit_expr(&args[1], &pointer)?;
        self.line("    move.l d0,a1");
        self.emit_expr(&args[2], &Type::Named("u24".to_owned()))?;
        self.line("    move.l d0,d2");
        let forward = self.next_label("intrinsic_copy_forward");
        let backward = self.next_label("intrinsic_copy_backward");
        let loop_label = self.next_label("intrinsic_copy_loop");
        let done = self.next_label("intrinsic_copy_done");
        if overlap_safe {
            self.line("    move.l a0,d3");
            self.line("    cmp.l a1,d3");
            self.line(&format!("    bls {forward}"));
            self.line(&format!("    bra {backward}"));
        }
        self.line(&format!("{forward}:"));
        self.line(&format!("{loop_label}:"));
        self.line("    tst.l d2");
        self.line(&format!("    beq {done}"));
        self.line("    move.b (a1)+,d0");
        self.line("    move.b d0,(a0)+");
        self.line("    subq.l #1,d2");
        self.line(&format!("    bra {loop_label}"));
        if overlap_safe {
            self.line(&format!("{backward}:"));
            self.line("    adda.l d2,a0");
            self.line("    adda.l d2,a1");
            self.line(&format!("{loop_label}_back:"));
            self.line("    tst.l d2");
            self.line(&format!("    beq {done}"));
            self.line("    move.b -(a1),d0");
            self.line("    move.b d0,-(a0)");
            self.line("    subq.l #1,d2");
            self.line(&format!("    bra {loop_label}_back"));
        }
        self.line(&format!("{done}:"));
        self.line("    moveq #0,d0");
        Ok(())
    }

    fn emit_fill_intrinsic(&mut self, args: &[Expr]) -> Result<(), Diagnostic> {
        self.require_arg_count(args, 3, "mem.fill")?;
        self.reject_known_volatile_memory(args, &[0])?;
        self.require_u8_pointer(&args[0])?;
        let pointer = Type::Ptr(Box::new(Type::Named("u8".to_owned())));
        self.emit_expr(&args[0], &pointer)?;
        self.line("    move.l d0,a0");
        self.emit_expr(&args[1], &Type::Named("u8".to_owned()))?;
        self.line("    move.b d0,d1");
        self.emit_expr(&args[2], &Type::Named("u24".to_owned()))?;
        self.line("    move.l d0,d2");
        let loop_label = self.next_label("intrinsic_fill_loop");
        let done = self.next_label("intrinsic_fill_done");
        self.line(&format!("{loop_label}:"));
        self.line("    tst.l d2");
        self.line(&format!("    beq {done}"));
        self.line("    move.b d1,(a0)+");
        self.line("    subq.l #1,d2");
        self.line(&format!("    bra {loop_label}"));
        self.line(&format!("{done}:"));
        self.line("    moveq #0,d0");
        Ok(())
    }

    fn emit_find_byte_intrinsic(&mut self, args: &[Expr]) -> Result<(), Diagnostic> {
        self.require_arg_count(args, 3, "mem.find_byte")?;
        self.reject_known_volatile_memory(args, &[0])?;
        self.require_u8_pointer(&args[0])?;
        let pointer = Type::Ptr(Box::new(Type::Named("u8".to_owned())));
        self.emit_expr(&args[0], &pointer)?;
        self.line("    move.l d0,a0");
        self.emit_expr(&args[1], &Type::Named("u24".to_owned()))?;
        self.line("    move.l d0,d2");
        self.emit_expr(&args[2], &Type::Named("u8".to_owned()))?;
        self.line("    move.b d0,d1");
        let loop_label = self.next_label("intrinsic_find_loop");
        let found = self.next_label("intrinsic_find_found");
        let not_found = self.next_label("intrinsic_find_not_found");
        self.line(&format!("{loop_label}:"));
        self.line("    tst.l d2");
        self.line(&format!("    beq {not_found}"));
        self.line("    cmp.b (a0),d1");
        self.line(&format!("    beq {found}"));
        self.line("    addq.l #1,a0");
        self.line("    subq.l #1,d2");
        self.line(&format!("    bra {loop_label}"));
        self.line(&format!("{found}:"));
        self.line("    move.l a0,d0");
        self.line("    moveq #1,d1");
        let done = self.next_label("intrinsic_find_done");
        self.line(&format!("    bra {done}"));
        self.line(&format!("{not_found}:"));
        self.line("    adda.l d2,a0");
        self.line("    move.l a0,d0");
        self.line("    moveq #0,d1");
        self.line(&format!("{done}:"));
        Ok(())
    }

    fn emit_compare_intrinsic(&mut self, args: &[Expr]) -> Result<(), Diagnostic> {
        self.require_arg_count(args, 3, "mem.compare")?;
        self.reject_known_volatile_memory(args, &[0, 1])?;
        self.require_u8_pointer(&args[0])?;
        self.require_u8_pointer(&args[1])?;
        let pointer = Type::Ptr(Box::new(Type::Named("u8".to_owned())));
        self.emit_expr(&args[0], &pointer)?;
        self.line("    move.l d0,a0");
        self.emit_expr(&args[1], &pointer)?;
        self.line("    move.l d0,a1");
        self.emit_expr(&args[2], &Type::Named("u24".to_owned()))?;
        self.line("    move.l d0,d2");
        let loop_label = self.next_label("intrinsic_compare_loop");
        let less = self.next_label("intrinsic_compare_less");
        let greater = self.next_label("intrinsic_compare_greater");
        let equal = self.next_label("intrinsic_compare_equal");
        let done = self.next_label("intrinsic_compare_done");
        self.line(&format!("{loop_label}:"));
        self.line("    tst.l d2");
        self.line(&format!("    beq {equal}"));
        self.line("    move.b (a0)+,d0");
        self.line("    move.b (a1)+,d1");
        self.line("    cmp.b d1,d0");
        self.line(&format!("    bcs {less}"));
        self.line(&format!("    bhi {greater}"));
        self.line("    subq.l #1,d2");
        self.line(&format!("    bra {loop_label}"));
        self.line(&format!("{less}:"));
        self.line("    moveq #-1,d0");
        self.line(&format!("    bra {done}"));
        self.line(&format!("{greater}:"));
        self.line("    moveq #1,d0");
        self.line(&format!("    bra {done}"));
        self.line(&format!("{equal}:"));
        self.line("    moveq #0,d0");
        self.line(&format!("{done}:"));
        Ok(())
    }

    fn emit_endian_load(
        &mut self,
        args: &[Expr],
        big_endian: bool,
        width: u8,
    ) -> Result<(), Diagnostic> {
        self.require_arg_count(args, 1, "endian load")?;
        self.reject_known_volatile_memory(args, &[0])?;
        self.require_u8_pointer(&args[0])?;
        let pointer = Type::Ptr(Box::new(Type::Named("u8".to_owned())));
        self.emit_expr(&args[0], &pointer)?;
        self.line("    move.l d0,a0");
        self.line("    moveq #0,d0");
        if big_endian {
            for offset in 0..width {
                self.line("    lsl.l #8,d0");
                self.line(&format!("    move.b {offset}(a0),d0"));
            }
        } else {
            self.line(&format!("    move.b {}(a0),d0", width - 1));
            for offset in (0..width - 1).rev() {
                self.line("    lsl.l #8,d0");
                self.line(&format!("    move.b {offset}(a0),d0"));
            }
        }
        self.mask_d0(width);
        Ok(())
    }

    fn emit_endian_store(
        &mut self,
        args: &[Expr],
        big_endian: bool,
        width: u8,
    ) -> Result<(), Diagnostic> {
        self.require_arg_count(args, 2, "endian store")?;
        self.reject_known_volatile_memory(args, &[0])?;
        self.require_u8_pointer(&args[0])?;
        let pointer = Type::Ptr(Box::new(Type::Named("u8".to_owned())));
        self.emit_expr(&args[0], &pointer)?;
        self.line("    move.l d0,a0");
        self.emit_expr(&args[1], &Type::Named(format!("u{}", width * 8)))?;
        self.mask_d0(width);
        self.line("    move.l d0,d1");
        if big_endian {
            for offset in 0..width {
                if offset != 0 {
                    self.line("    lsr.l #8,d1");
                }
                self.line(&format!("    move.b d1,{}(a0)", width - 1 - offset));
            }
        } else {
            for offset in 0..width {
                if offset != 0 {
                    self.line("    lsr.l #8,d1");
                }
                self.line(&format!("    move.b d1,{offset}(a0)"));
            }
        }
        self.line("    moveq #0,d0");
        Ok(())
    }

    fn reserve_call_arguments(
        &mut self,
        args: &[Expr],
        params: &[Type],
    ) -> Result<u32, Diagnostic> {
        let mut argument_bytes = 0_u32;
        for ty in params {
            argument_bytes = argument_bytes
                .checked_add(abi_slot_size(&self.model, ty)?)
                .ok_or_else(|| Diagnostic::new("M68k call argument area is too large"))?;
        }
        if argument_bytes > i32::from(i16::MAX) as u32 {
            return Err(Diagnostic::new(
                "M68k call argument area exceeds the signed stack displacement",
            ));
        }
        if argument_bytes != 0 {
            self.line(&format!("    suba.l #{argument_bytes},sp"));
        }
        let mut offset = 0_i32;
        for (arg, ty) in args.iter().zip(params) {
            let ty = self.model.resolved_type(ty)?;
            self.emit_expr(arg, &ty)?;
            self.store_ref_d0(MemoryRef::Stack(offset), &ty)?;
            offset += i32::try_from(abi_slot_size(&self.model, &ty)?)
                .map_err(|_| Diagnostic::new("M68k call argument area is too large"))?;
        }
        Ok(argument_bytes)
    }

    fn release_call_arguments(
        &mut self,
        argument_bytes: u32,
        hidden_pointer: bool,
    ) -> Result<(), Diagnostic> {
        if argument_bytes != 0 {
            self.line(&format!("    adda.l #{argument_bytes},sp"));
        }
        if hidden_pointer {
            self.line("    adda.l #4,sp");
        }
        Ok(())
    }

    fn emit_call(
        &mut self,
        path: &[String],
        args: &[Expr],
        expected: Option<&Type>,
    ) -> Result<(), Diagnostic> {
        let name = path.join(".");
        if let Some(operation) = intrinsic_operation(path) {
            return self.emit_intrinsic(operation, args, expected);
        }
        let direct_signature = self
            .model
            .functions
            .get(&name)
            .or_else(|| {
                path.last()
                    .and_then(|short| self.model.functions.get(short))
            })
            .cloned();
        let (signature, indirect) = if let Some(signature) = direct_signature {
            (signature, None)
        } else {
            if path.len() != 1 {
                return Err(Diagnostic::new(format!("unknown function `{name}`")));
            }
            let binding = self.binding(&path[0])?;
            let pointer_type = self.model.resolved_type(&binding.ty)?;
            let Type::Ptr(inner) = pointer_type.clone() else {
                return Err(Diagnostic::new(format!(
                    "M68k function pointer call requires `ptr<fn(...)>`, got `{pointer_type:?}`"
                )));
            };
            let Type::Function {
                params,
                return_type,
            } = *inner
            else {
                return Err(Diagnostic::new(format!(
                    "M68k function pointer call requires `ptr<fn(...)>`, got `{pointer_type:?}`"
                )));
            };
            (
                crate::tbir::model::FunctionSignature {
                    params,
                    return_type: return_type.map(|ty| *ty),
                    second_return_type: None,
                    argument_slots: Vec::new(),
                },
                Some(binding),
            )
        };
        if signature.second_return_type.is_some() {
            return Err(Diagnostic::new(format!(
                "two-result function `{name}` requires a two-result binding"
            )));
        }
        if signature.params.len() != args.len() {
            return Err(Diagnostic::new(format!(
                "function `{name}` expects {} arguments, got {}",
                signature.params.len(),
                args.len()
            )));
        }

        let mut hidden_pointer = false;
        if let Some(binding) = indirect.as_ref() {
            self.emit_expr(&Expr::Ident(name.clone()), &binding.ty)?;
            self.line("    move.l d0,-(sp)");
            hidden_pointer = true;
        }
        let argument_bytes = self.reserve_call_arguments(args, &signature.params)?;
        if let Some(_) = indirect {
            let pointer_offset = i32::try_from(argument_bytes)
                .map_err(|_| Diagnostic::new("M68k call argument area is too large"))?;
            self.line(&format!("    move.l {pointer_offset}(sp),a0"));
            self.line("    jsr (a0)");
        } else {
            self.line(&format!("    jsr ({}).l", function_label(&name)));
        }
        self.release_call_arguments(argument_bytes, hidden_pointer)?;
        if let Some(expected) = expected {
            if signature.return_type.is_none() {
                self.line("    moveq #0,d0");
            } else {
                self.require_scalar(expected)?;
            }
        }
        Ok(())
    }

    fn emit_return_two(&mut self, first: &Expr, second: &Expr) -> Result<(), Diagnostic> {
        let first_ty = self
            .return_types
            .last()
            .and_then(Clone::clone)
            .ok_or_else(|| {
                Diagnostic::new("function cannot return two values without a first return type")
            })?;
        let second_ty = self
            .second_return_types
            .last()
            .and_then(Clone::clone)
            .ok_or_else(|| Diagnostic::new("function cannot return two values"))?;
        self.require_scalar(&first_ty)?;
        self.require_scalar(&second_ty)?;
        self.emit_expr(first, &first_ty)?;
        self.line("    move.l d0,-(sp)");
        self.emit_expr(second, &second_ty)?;
        self.line("    move.l d0,d1");
        self.line("    move.l (sp)+,d0");
        let label = self
            .return_labels
            .last()
            .expect("function return label")
            .clone();
        self.line(&format!("    bra {label}"));
        Ok(())
    }

    fn emit_two_result_call(
        &mut self,
        path: &[String],
        args: &[Expr],
        first: &Binding,
        second: &Binding,
    ) -> Result<(), Diagnostic> {
        if let Some(operation) = intrinsic_operation(path) {
            self.emit_intrinsic_pair(operation, args, first, second)?;
            return Ok(());
        }
        let name = path.join(".");
        let signature = self
            .model
            .functions
            .get(&name)
            .or_else(|| {
                path.last()
                    .and_then(|short| self.model.functions.get(short))
            })
            .cloned()
            .ok_or_else(|| Diagnostic::new(format!("unknown function `{name}`")))?;
        let first_return_type = signature.return_type.clone().ok_or_else(|| {
            Diagnostic::new(format!("function `{name}` does not return two values"))
        })?;
        let second_return_type = signature.second_return_type.clone().ok_or_else(|| {
            Diagnostic::new(format!("function `{name}` does not return two values"))
        })?;
        self.require_scalar(&first_return_type)?;
        self.require_scalar(&second_return_type)?;
        self.require_scalar(&first.ty)?;
        self.require_scalar(&second.ty)?;
        if signature.params.len() != args.len() {
            return Err(Diagnostic::new(format!(
                "function `{name}` expects {} arguments, got {}",
                signature.params.len(),
                args.len()
            )));
        }
        let argument_bytes = self.reserve_call_arguments(args, &signature.params)?;
        self.line(&format!("    jsr ({}).l", function_label(&name)));
        self.release_call_arguments(argument_bytes, false)?;
        self.store_binding_d1(second)?;
        self.store_binding_d0(first)?;
        Ok(())
    }

    fn emit_intrinsic_pair(
        &mut self,
        operation: IntrinsicOperation,
        args: &[Expr],
        first: &Binding,
        second: &Binding,
    ) -> Result<(), Diagnostic> {
        let result = intrinsic_result_type(operation, args, |arg| self.expr_type(arg))?;
        if result.len() != 2 {
            return Err(Diagnostic::new("intrinsic does not produce two results"));
        }
        let first_ty = self.model.resolved_type(&first.ty)?;
        let second_ty = self.model.resolved_type(&second.ty)?;
        if self.model.resolved_type(&result[0])? != first_ty
            || self.model.resolved_type(&result[1])? != second_ty
        {
            return Err(Diagnostic::new(
                "intrinsic result types do not match the two-result binding",
            ));
        }
        match operation {
            IntrinsicOperation::Int(IntIntrinsic::Divmod) => self.emit_divmod_intrinsic(args)?,
            IntrinsicOperation::Int(IntIntrinsic::AddCarry) => {
                self.emit_add_sub_carry_intrinsic(args, false)?
            }
            IntrinsicOperation::Int(IntIntrinsic::SubBorrow) => {
                self.emit_add_sub_carry_intrinsic(args, true)?
            }
            IntrinsicOperation::Int(IntIntrinsic::FullMul) => self.emit_full_mul_intrinsic(args)?,
            IntrinsicOperation::Mem(MemIntrinsic::FindByte) => {
                self.emit_find_byte_intrinsic(args)?
            }
            _ => {
                return Err(Diagnostic::new(
                    "intrinsic is not a supported paired operation",
                ));
            }
        }
        self.store_binding_d1(second)?;
        self.store_binding_d0(first)?;
        Ok(())
    }

    fn emit_divmod_intrinsic(&mut self, args: &[Expr]) -> Result<(), Diagnostic> {
        self.require_arg_count(args, 2, "int.divmod")?;
        let ty = self.model.resolved_type(&self.expr_type(&args[0])?)?;
        let other = self.model.resolved_type(&self.expr_type(&args[1])?)?;
        if ty != other
            || !matches!(ty, Type::Named(ref name) if name.starts_with('u') || name.starts_with('i'))
        {
            return Err(Diagnostic::new(
                "int.divmod operands must have the same integer type",
            ));
        }
        let width = scalar_width(&ty)?;
        self.emit_expr(&args[0], &ty)?;
        self.line("    move.l d0,-(sp)");
        self.emit_expr(&args[1], &ty)?;
        self.line("    move.l d0,d2");
        self.line("    move.l (sp)+,d0");
        if width <= 2 {
            let zero = self.next_label("intrinsic_div_zero");
            let done = self.next_label("intrinsic_div_done");
            self.line("    tst.w d2");
            self.line(&format!("    beq {zero}"));
            if type_is_signed(&ty) {
                self.extend_signed_to_long(&ty)?;
                self.extend_register_signed("d2", &ty)?;
                self.line("    divs.w d2,d0");
            } else {
                self.mask_d0(width);
                self.mask_register("d2", width);
                self.line("    divu.w d2,d0");
            }
            self.line("    move.l d0,d1");
            self.line("    swap d1");
            self.mask_d0(width);
            self.mask_register("d1", width);
            self.line(&format!("    bra {done}"));
            self.line(&format!("{zero}:"));
            self.line("    moveq #0,d0");
            self.line("    moveq #0,d1");
            self.line(&format!("{done}:"));
            return Ok(());
        }
        self.mask_d0(width);
        self.mask_register("d2", width);
        self.emit_wide_divmod(&ty)
    }

    fn emit_wide_divmod(&mut self, ty: &Type) -> Result<(), Diagnostic> {
        let signed = type_is_signed(ty);
        let width = u32::from(scalar_width(ty)?) * 8;
        let divisor_zero = self.next_label("intrinsic_div_wide_zero");
        let loop_label = self.next_label("intrinsic_div_wide_loop");
        let result_ready = self.next_label("intrinsic_div_wide_ready");
        let positive_quotient = self.next_label("intrinsic_div_wide_q_positive");
        let positive_remainder = self.next_label("intrinsic_div_wide_r_positive");
        self.line("    moveq #0,d5");
        if signed {
            let left_positive = self.next_label("intrinsic_div_wide_left_positive");
            let right_positive = self.next_label("intrinsic_div_wide_right_positive");
            self.line("    tst.l d0");
            self.line(&format!("    bpl {left_positive}"));
            self.line("    neg.l d0");
            self.line("    moveq #3,d5");
            self.line(&format!("{left_positive}:"));
            self.line("    tst.l d2");
            self.line(&format!("    bpl {right_positive}"));
            self.line("    neg.l d2");
            self.line("    eori.b #1,d5");
            self.line(&format!("{right_positive}:"));
        }
        self.line("    tst.l d2");
        self.line(&format!("    beq {divisor_zero}"));
        self.line("    moveq #0,d1");
        self.line(&format!("    moveq #{width},d4"));
        self.line(&format!("{loop_label}:"));
        self.line("    move.l d0,d6");
        self.line("    rol.l #1,d6");
        self.line("    andi.l #1,d6");
        self.line("    move.l d1,d3");
        self.line("    rol.l #1,d3");
        self.line("    andi.l #1,d3");
        self.line("    add.l d0,d0");
        self.line("    add.l d1,d1");
        self.line("    add.l d6,d1");
        self.line("    move.l d1,d6");
        self.line("    eori.l #$80000000,d6");
        self.line("    move.l d2,d7");
        self.line("    eori.l #$80000000,d7");
        self.line("    cmp.l d7,d6");
        self.line("    sge d6");
        self.line("    andi.l #1,d6");
        self.line("    or.l d3,d6");
        self.line("    neg.l d6");
        self.line("    move.l d2,d3");
        self.line("    and.l d6,d3");
        self.line("    sub.l d3,d1");
        self.line("    andi.l #1,d6");
        self.line("    add.l d6,d0");
        self.line("    subq.w #1,d4");
        self.line(&format!("    bne {loop_label}"));
        self.line(&format!("    bra {result_ready}"));
        self.line(&format!("{divisor_zero}:"));
        self.line("    moveq #0,d0");
        self.line("    moveq #0,d1");
        self.line(&format!("    bra {positive_quotient}"));
        self.line(&format!("{result_ready}:"));
        if signed {
            self.line("    btst #0,d5");
            self.line(&format!("    beq {positive_quotient}"));
            self.line("    neg.l d0");
            self.line(&format!("{positive_quotient}:"));
            self.line("    btst #1,d5");
            self.line(&format!("    beq {positive_remainder}"));
            self.line("    neg.l d1");
            self.line(&format!("{positive_remainder}:"));
        } else {
            self.line(&format!("{positive_quotient}:"));
        }
        self.mask_d0(scalar_width(ty)?);
        self.mask_register("d1", scalar_width(ty)?);
        Ok(())
    }

    fn emit_add_sub_carry_intrinsic(
        &mut self,
        args: &[Expr],
        subtract: bool,
    ) -> Result<(), Diagnostic> {
        self.require_arg_count(
            args,
            3,
            if subtract {
                "int.sub_borrow"
            } else {
                "int.add_carry"
            },
        )?;
        let ty = self.model.resolved_type(&self.expr_type(&args[0])?)?;
        let other = self.model.resolved_type(&self.expr_type(&args[1])?)?;
        if ty != other
            || !matches!(ty, Type::Named(ref name) if name.starts_with('u') || name.starts_with('i'))
        {
            return Err(Diagnostic::new(
                "carry/borrow operands must have the same integer type",
            ));
        }
        let width = scalar_width(&ty)?;
        let suffix = size_suffix(&ty)?;
        self.emit_expr(&args[0], &ty)?;
        self.line("    move.l d0,-(sp)");
        self.emit_expr(&args[1], &ty)?;
        self.line("    move.l d0,d2");
        self.line("    move.l (sp)+,d0");
        self.line("    move.l d0,-(sp)");
        self.emit_expr(&args[2], &Type::Named("bool".to_owned()))?;
        self.line("    move.l d0,d3");
        self.line("    move.l (sp)+,d0");
        if width == 3 {
            self.mask_d0(width);
            self.mask_register("d2", width);
        }
        return self.emit_add_sub_carry_core(suffix, width, subtract);
    }

    fn emit_add_sub_carry_core(
        &mut self,
        suffix: &str,
        width: u8,
        subtract: bool,
    ) -> Result<(), Diagnostic> {
        // d0 is the left operand, d2 the right operand, and d3 the carry/borrow input.
        let set = self.next_label("intrinsic_carry_set");
        let done = self.next_label("intrinsic_carry_done");
        let no_initial = self.next_label("intrinsic_carry_no_initial");
        self.line("    moveq #0,d1");
        if width == 3 {
            if subtract {
                self.line("    sub.l d2,d0");
            } else {
                self.line("    add.l d2,d0");
            }
            self.line("    move.l d0,d4");
            self.line("    andi.l #$FF000000,d4");
            self.line(&format!("    beq {no_initial}"));
            self.line("    moveq #1,d1");
            self.line(&format!("{no_initial}:"));
            self.line("    andi.l #$FFFFFF,d0");
            self.line("    tst.b d3");
            self.line(&format!("    beq {done}"));
            if subtract {
                self.line("    subq.l #1,d0");
            } else {
                self.line("    addq.l #1,d0");
            }
            self.line("    move.l d0,d4");
            self.line("    andi.l #$FF000000,d4");
            self.line(&format!("    beq {done}"));
            self.line(&format!("    bra {set}"));
            self.line(&format!("{set}:"));
            self.line("    moveq #1,d1");
            self.line(&format!("{done}:"));
            self.mask_d0(width);
            return Ok(());
        }
        if subtract {
            self.line(&format!("    sub.{suffix} d2,d0"));
        } else {
            self.line(&format!("    add.{suffix} d2,d0"));
        }
        self.line(&format!("    bcc {no_initial}"));
        self.line("    moveq #1,d1");
        self.line(&format!("{no_initial}:"));
        self.line("    tst.b d3");
        self.line(&format!("    beq {done}"));
        if subtract {
            self.line(&format!("    subq.{suffix} #1,d0"));
        } else {
            self.line(&format!("    addq.{suffix} #1,d0"));
        }
        self.line(&format!("    bcc {done}"));
        self.line(&format!("{set}:"));
        self.line("    moveq #1,d1");
        self.line(&format!("{done}:"));
        self.mask_d0(width);
        Ok(())
    }

    fn load_place(&mut self, place: &Place, ty: &Type) -> Result<(), Diagnostic> {
        match place {
            Place::Ident(name) => self.load_binding_d0(&self.binding(name)?),
            Place::Index { name, index } => {
                self.emit_named_index_address(name, index)?;
                self.load_from_a0(ty)
            }
            Place::Field { base, field } => {
                let binding = self.binding(base)?;
                let field = self.model.field(&binding.ty, field)?.clone();
                self.load_d0(
                    Storage {
                        address: binding.memory()?.address + field.offset,
                        size: field.size,
                    },
                    ty,
                )
            }
            Place::Access(path) => {
                self.emit_access_address(path)?;
                self.load_from_a0(ty)
            }
            Place::Deref(expr) => {
                let pointer_ty = self.expr_type(expr)?;
                self.emit_expr(expr, &pointer_ty)?;
                self.line("    move.l d0,a0");
                self.line(&format!("    move.{} (a0),d0", size_suffix(ty)?));
                Ok(())
            }
        }
    }

    fn store_place(&mut self, place: &Place, ty: &Type) -> Result<(), Diagnostic> {
        match place {
            Place::Ident(name) => self.store_binding_d0(&self.binding(name)?),
            Place::Index { name, index } => {
                self.line(&format!("    move.{} d0,d3", size_suffix(ty)?));
                self.emit_named_index_address(name, index)?;
                self.line(&format!("    move.{} d3,d0", size_suffix(ty)?));
                self.store_to_a0(ty)
            }
            Place::Field { base, field } => {
                let binding = self.binding(base)?;
                let field = self.model.field(&binding.ty, field)?.clone();
                self.store_d0(
                    Storage {
                        address: binding.memory()?.address + field.offset,
                        size: field.size,
                    },
                    ty,
                )
            }
            Place::Access(path) => {
                self.line(&format!("    move.{} d0,d3", size_suffix(ty)?));
                self.emit_access_address(path)?;
                self.line(&format!("    move.{} d3,d0", size_suffix(ty)?));
                self.store_to_a0(ty)
            }
            Place::Deref(expr) => {
                self.line(&format!("    move.{} d0,d3", size_suffix(ty)?));
                let pointer_ty = self.expr_type(expr)?;
                self.emit_expr(expr, &pointer_ty)?;
                self.line("    move.l d0,a0");
                self.line(&format!("    move.{} d3,(a0)", size_suffix(ty)?));
                Ok(())
            }
        }
    }

    fn store_aggregate_place(
        &mut self,
        place: &Place,
        source: Storage,
        size: u32,
    ) -> Result<(), Diagnostic> {
        match place {
            Place::Ident(name) => {
                let destination = self.binding(name)?;
                self.copy(source, destination.memory()?, size);
            }
            Place::Index { name, index } => {
                self.emit_named_index_address(name, index)?;
                self.copy_to_a0(source, size);
            }
            Place::Field { base, field } => {
                let binding = self.binding(base)?;
                let field = self.model.field(&binding.ty, field)?;
                self.copy(
                    source,
                    Storage {
                        address: binding.memory()?.address + field.offset,
                        size,
                    },
                    size,
                );
            }
            Place::Access(path) => {
                self.emit_access_address(path)?;
                self.copy_to_a0(source, size);
            }
            Place::Deref(expr) => {
                let pointer_ty = self.expr_type(expr)?;
                self.emit_expr(expr, &pointer_ty)?;
                self.line("    move.l d0,a0");
                self.copy_to_a0(source, size);
            }
        }
        Ok(())
    }

    fn place_type(&self, place: &Place) -> Result<Type, Diagnostic> {
        match place {
            Place::Ident(name) => Ok(self.binding(name)?.ty),
            Place::Index { name, .. } => {
                element_type(&self.model.resolved_type(&self.binding(name)?.ty)?)
            }
            Place::Field { base, field } => {
                Ok(self.model.field(&self.binding(base)?.ty, field)?.ty.clone())
            }
            Place::Access(path) => self.access_type(path),
            Place::Deref(expr) => match self.model.resolved_type(&self.expr_type(expr)?)? {
                Type::Ptr(inner) => Ok(*inner),
                _ => Err(Diagnostic::new("dereference requires pointer")),
            },
        }
    }

    fn emit_named_index_address(&mut self, name: &str, index: &Expr) -> Result<Type, Diagnostic> {
        let binding = self.binding(name)?;
        let resolved = self.model.resolved_type(&binding.ty)?;
        let element = element_type(&resolved)?;
        let element_size = self.model.type_size(&element)?;
        match resolved {
            Type::Array { .. } => {
                self.load_storage_address(binding.memory()?, "a0")?;
            }
            Type::Ptr(_) => match binding.location {
                BindingLocation::Memory(storage) => {
                    self.load_d0(storage, &binding.ty)?;
                    self.line("    move.l d0,a0");
                }
                BindingLocation::AddressRegister(register) => {
                    self.line(&format!("    move.l a{register},a0"));
                }
            },
            _ => return Err(Diagnostic::new("indexing requires array or pointer")),
        }
        self.add_index_to_a0(index, element_size)?;
        Ok(element)
    }

    fn emit_access_address(&mut self, path: &AccessPath) -> Result<Type, Diagnostic> {
        let binding = self.binding(&path.root)?;
        let mut ty = self.model.resolved_type(&binding.ty)?;
        if let Type::Ptr(inner) = ty {
            match binding.location {
                BindingLocation::Memory(storage) => {
                    self.load_d0(storage, &binding.ty)?;
                    self.line("    move.l d0,a0");
                }
                BindingLocation::AddressRegister(register) => {
                    self.line(&format!("    move.l a{register},a0"));
                }
            }
            ty = *inner;
        } else {
            self.load_storage_address(binding.memory()?, "a0")?;
        }
        for segment in &path.segments {
            match segment {
                AccessSegment::Field(name) => {
                    let field = self.model.field(&ty, name)?.clone();
                    if field.offset != 0 {
                        self.line(&format!("    lea {}(a0),a0", field.offset));
                    }
                    ty = field.ty;
                }
                AccessSegment::Index(index) => {
                    let element = element_type(&self.model.resolved_type(&ty)?)?;
                    self.add_index_to_a0(index, self.model.type_size(&element)?)?;
                    ty = element;
                }
            }
        }
        Ok(ty)
    }

    fn add_index_to_a0(&mut self, index: &Expr, element_size: u32) -> Result<(), Diagnostic> {
        if let Ok(index) = self.model.const_value(index) {
            let offset = u32::try_from(index)
                .ok()
                .and_then(|index| index.checked_mul(element_size))
                .ok_or_else(|| Diagnostic::new("array index offset overflow"))?;
            if offset != 0 {
                self.line(&format!("    lea {offset}(a0),a0"));
            }
            return Ok(());
        }
        self.emit_expr(index, &Type::Named("u24".to_owned()))?;
        if element_size > 1 {
            self.line(&format!("    mulu #${element_size:X},d0"));
        }
        self.line("    adda.l d0,a0");
        Ok(())
    }

    fn load_from_a0(&mut self, ty: &Type) -> Result<(), Diagnostic> {
        if scalar_width(ty)? == 3 {
            self.line("    moveq #0,d0");
            self.line("    move.b (a0),d0");
            self.line("    lsl.l #8,d0");
            self.line("    move.b 1(a0),d0");
            self.line("    lsl.l #8,d0");
            self.line("    move.b 2(a0),d0");
            self.normalize_d0(ty)?;
        } else {
            self.line(&format!("    move.{} (a0),d0", size_suffix(ty)?));
        }
        Ok(())
    }

    fn store_to_a0(&mut self, ty: &Type) -> Result<(), Diagnostic> {
        if scalar_width(ty)? == 3 {
            self.line("    move.l d0,d1");
            self.line("    lsr.l #8,d1");
            self.line("    lsr.l #8,d1");
            self.line("    move.b d1,(a0)");
            self.line("    move.l d0,d1");
            self.line("    lsr.l #8,d1");
            self.line("    move.b d1,1(a0)");
            self.line("    move.b d0,2(a0)");
        } else {
            self.line(&format!("    move.{} d0,(a0)", size_suffix(ty)?));
        }
        Ok(())
    }

    fn copy(&mut self, source: Storage, target: Storage, size: u32) {
        let source = self
            .storage_ref(source)
            .expect("source storage is addressable");
        let target = self
            .storage_ref(target)
            .expect("target storage is addressable");
        self.copy_refs(source, target, size);
    }

    fn copy_refs(&mut self, source: MemoryRef, target: MemoryRef, size: u32) {
        for offset in 0..size {
            self.line(&format!(
                "    move.b {},d0",
                self.memory_operand(source.offset(offset as i32))
                    .expect("source memory operand")
            ));
            self.line(&format!(
                "    move.b d0,{}",
                self.memory_operand(target.offset(offset as i32))
                    .expect("target memory operand")
            ));
        }
    }

    fn zero(&mut self, storage: Storage) {
        let reference = self
            .storage_ref(storage)
            .expect("zero storage is addressable");
        self.line("    moveq #0,d0");
        for offset in 0..storage.size {
            self.line(&format!(
                "    move.b d0,{}",
                self.memory_operand(reference.offset(offset as i32))
                    .expect("zero memory operand")
            ));
        }
    }

    fn copy_from_a0(&mut self, target: Storage, size: u32) {
        let target = self
            .storage_ref(target)
            .expect("target storage is addressable");
        for offset in 0..size {
            self.line(&format!("    move.b {offset}(a0),d0"));
            self.line(&format!(
                "    move.b d0,{}",
                self.memory_operand(target.offset(offset as i32))
                    .expect("target memory operand")
            ));
        }
    }

    fn copy_to_a0(&mut self, source: Storage, size: u32) {
        let source = self
            .storage_ref(source)
            .expect("source storage is addressable");
        for offset in 0..size {
            self.line(&format!(
                "    move.b {},d0",
                self.memory_operand(source.offset(offset as i32))
                    .expect("source memory operand")
            ));
            self.line(&format!("    move.b d0,{offset}(a0)"));
        }
    }

    fn expr_type(&self, expr: &Expr) -> Result<Type, Diagnostic> {
        match expr {
            Expr::Int(value) => Ok(if (0..=0xFF).contains(value) {
                Type::Named("u8".to_owned())
            } else {
                Type::Named("u16".to_owned())
            }),
            Expr::TypedInt(_, ty) | Expr::Cast { ty, .. } => self.model.resolved_type(ty),
            Expr::Bool(_) => Ok(Type::Named("bool".to_owned())),
            Expr::Char(_) | Expr::In(_) => Ok(Type::Named("u8".to_owned())),
            Expr::String(_) => Ok(Type::Ptr(Box::new(Type::Named("u8".to_owned())))),
            Expr::Ident(name) => self
                .model
                .constant_types
                .get(name)
                .cloned()
                .or_else(|| self.binding(name).ok().map(|binding| binding.ty))
                .ok_or_else(|| Diagnostic::new(format!("unknown value `{name}`"))),
            Expr::Index { name, .. } => {
                element_type(&self.model.resolved_type(&self.binding(name)?.ty)?)
            }
            Expr::Field { base, field } => self
                .model
                .constant_types
                .get(&format!("{base}.{field}"))
                .cloned()
                .or_else(|| {
                    self.model
                        .field(&self.binding(base).ok()?.ty, field)
                        .ok()
                        .map(|field| field.ty.clone())
                })
                .ok_or_else(|| Diagnostic::new(format!("unknown field `{field}"))),
            Expr::AddressOfIndex { name, .. } => Ok(Type::Ptr(Box::new(element_type(
                &self.model.resolved_type(&self.binding(name)?.ty)?,
            )?))),
            Expr::AddressOfField { base, field } => Ok(Type::Ptr(Box::new(
                self.model.field(&self.binding(base)?.ty, field)?.ty.clone(),
            ))),
            Expr::Access(path) => self.access_type(path),
            Expr::AddressOfAccess(path) => Ok(Type::Ptr(Box::new(self.access_type(path)?))),
            Expr::AddressOf(name) => {
                if let Some(function_type) = self.function_value_type(name) {
                    Ok(Type::Ptr(Box::new(function_type)))
                } else {
                    Ok(Type::Ptr(Box::new(self.binding(name)?.ty)))
                }
            }
            Expr::StructInit { ty, .. } => Ok(Type::Named(ty.clone())),
            Expr::Call { path, args } => {
                if let Some(operation) = intrinsic_operation(path) {
                    return intrinsic_result_type(operation, args, |arg| self.expr_type(arg))?
                        .into_iter()
                        .next()
                        .ok_or_else(|| Diagnostic::new("intrinsic has no scalar result"));
                }
                if let Some(signature) = self
                    .model
                    .functions
                    .get(&path.join("."))
                    .or_else(|| path.last().and_then(|name| self.model.functions.get(name)))
                {
                    return signature
                        .return_type
                        .clone()
                        .ok_or_else(|| Diagnostic::new("void function has no value"));
                }
                let name = path
                    .last()
                    .ok_or_else(|| Diagnostic::new("empty function call path"))?;
                let pointer_type = self.model.resolved_type(&self.binding(name)?.ty)?;
                let Type::Ptr(inner) = pointer_type else {
                    return Err(Diagnostic::new(
                        "function pointer call requires a function pointer",
                    ));
                };
                let Type::Function { return_type, .. } = *inner else {
                    return Err(Diagnostic::new(
                        "function pointer call requires a function pointer",
                    ));
                };
                return return_type
                    .map(|ty| *ty)
                    .ok_or_else(|| Diagnostic::new("void function has no value"));
            }
            Expr::Unary {
                op: UnaryOp::Not, ..
            } => Ok(Type::Named("bool".to_owned())),
            Expr::Unary { expr, .. } => self.expr_type(expr),
            Expr::BankedPointer { pointer, .. } => self.expr_type(pointer),
            Expr::Deref(expr) => match self.model.resolved_type(&self.expr_type(expr)?)? {
                Type::Ptr(inner) => Ok(*inner),
                _ => Err(Diagnostic::new("dereference requires pointer")),
            },
            Expr::Binary { left, op, .. }
                if is_comparison(*op) || matches!(op, BinaryOp::And | BinaryOp::Or) =>
            {
                Ok(Type::Named("bool".to_owned()))
            }
            Expr::Binary { left, .. } => self.expr_type(left),
            Expr::Array(_) => Err(Diagnostic::new("array type requires context")),
        }
    }

    fn access_type(&self, path: &AccessPath) -> Result<Type, Diagnostic> {
        let mut ty = self.model.resolved_type(&self.binding(&path.root)?.ty)?;
        if let Type::Ptr(inner) = ty {
            ty = *inner;
        }
        for segment in &path.segments {
            ty = match segment {
                AccessSegment::Field(name) => self.model.field(&ty, name)?.ty.clone(),
                AccessSegment::Index(_) => element_type(&self.model.resolved_type(&ty)?)?,
            };
        }
        Ok(ty)
    }

    fn emit_inline_asm(
        &mut self,
        inputs: &[crate::ast::AsmInput],
        outputs: &[crate::ast::AsmOutput],
        lines: &[String],
    ) -> Result<(), Diagnostic> {
        let mut operands = HashMap::new();
        for operand in inputs
            .iter()
            .map(|input| &input.name)
            .chain(outputs.iter().map(|output| &output.name))
        {
            let binding = self.binding(operand)?;
            let storage = binding.memory()?;
            operands.insert(
                operand.clone(),
                self.memory_operand(self.storage_ref(storage)?)?,
            );
        }
        for line in lines {
            let mut emitted = line.clone();
            for (name, value) in &operands {
                emitted = emitted.replace(&format!("{{{name}}}"), value);
            }
            if emitted.contains(['{', '}']) {
                return Err(Diagnostic::new(format!(
                    "unknown inline asm operand placeholder in `{line}`"
                )));
            }
            self.line(&format!("    {emitted}"));
        }
        Ok(())
    }

    fn boolean_from_test(&mut self, branch: &str, ty: &Type) -> Result<(), Diagnostic> {
        self.line(&format!("    tst.{} d0", size_suffix(ty)?));
        self.boolean_from_branch(branch)
    }

    fn boolean_from_branch(&mut self, branch: &str) -> Result<(), Diagnostic> {
        let yes = self.next_label("true");
        let done = self.next_label("bool_done");
        self.line(&format!("    {branch} {yes}"));
        self.line("    moveq #0,d0");
        self.line(&format!("    bra {done}"));
        self.line(&format!("{yes}:"));
        self.line("    moveq #1,d0");
        self.line(&format!("{done}:"));
        Ok(())
    }

    fn load_constant(&mut self, value: i64, ty: &Type) -> Result<(), Diagnostic> {
        let (size, mask) = match scalar_width(ty)? {
            1 => ("b", 0xFF),
            2 => ("w", 0xFFFF),
            3 => ("l", 0xFF_FFFF),
            4 => ("l", 0xFFFF_FFFF),
            _ => unreachable!(),
        };
        self.line(&format!("    move.{size} #${:X},d0", (value as u64) & mask));
        self.normalize_d0(ty)?;
        Ok(())
    }

    fn load_d0(&mut self, storage: Storage, ty: &Type) -> Result<(), Diagnostic> {
        self.load_ref_d0(self.storage_ref(storage)?, ty)
    }

    fn load_ref_d0(&mut self, reference: MemoryRef, ty: &Type) -> Result<(), Diagnostic> {
        if scalar_width(ty)? == 3 {
            self.line("    moveq #0,d0");
            self.line(&format!(
                "    move.b {},d0",
                self.memory_operand(reference)?
            ));
            self.line("    lsl.l #8,d0");
            self.line(&format!(
                "    move.b {},d0",
                self.memory_operand(reference.offset(1))?
            ));
            self.line("    lsl.l #8,d0");
            self.line(&format!(
                "    move.b {},d0",
                self.memory_operand(reference.offset(2))?
            ));
            self.normalize_d0(ty)?;
        } else {
            self.line(&format!(
                "    move.{} {},d0",
                size_suffix(ty)?,
                self.memory_operand(reference)?
            ));
        }
        Ok(())
    }

    fn load_binding_d0(&mut self, binding: &Binding) -> Result<(), Diagnostic> {
        match binding.location {
            BindingLocation::Memory(storage) => self.load_d0(storage, &binding.ty),
            BindingLocation::AddressRegister(register) => {
                if !matches!(binding.ty, Type::Ptr(_)) {
                    return Err(Diagnostic::new(format!(
                        "non-pointer local was allocated to address register a{register}"
                    )));
                }
                self.line(&format!("    move.l a{register},d0"));
                self.normalize_d0(&binding.ty)
            }
        }
    }

    fn store_binding_d0(&mut self, binding: &Binding) -> Result<(), Diagnostic> {
        match binding.location {
            BindingLocation::Memory(storage) => self.store_d0(storage, &binding.ty),
            BindingLocation::AddressRegister(register) => {
                if !matches!(binding.ty, Type::Ptr(_)) {
                    return Err(Diagnostic::new(format!(
                        "non-pointer local was allocated to address register a{register}"
                    )));
                }
                self.line(&format!("    move.l d0,a{register}"));
                Ok(())
            }
        }
    }

    fn store_d0(&mut self, storage: Storage, ty: &Type) -> Result<(), Diagnostic> {
        self.store_ref_d0(self.storage_ref(storage)?, ty)
    }

    fn store_ref_d0(&mut self, reference: MemoryRef, ty: &Type) -> Result<(), Diagnostic> {
        if scalar_width(ty)? == 3 {
            self.line("    move.l d0,d1");
            self.line("    lsr.l #8,d1");
            self.line("    lsr.l #8,d1");
            self.line(&format!(
                "    move.b d1,{}",
                self.memory_operand(reference)?
            ));
            self.line("    move.l d0,d1");
            self.line("    lsr.l #8,d1");
            self.line(&format!(
                "    move.b d1,{}",
                self.memory_operand(reference.offset(1))?
            ));
            self.line(&format!(
                "    move.b d0,{}",
                self.memory_operand(reference.offset(2))?
            ));
        } else {
            self.line(&format!(
                "    move.{} d0,{}",
                size_suffix(ty)?,
                self.memory_operand(reference)?
            ));
        }
        Ok(())
    }

    fn store_binding_d1(&mut self, binding: &Binding) -> Result<(), Diagnostic> {
        match binding.location {
            BindingLocation::Memory(storage) => self.store_d1(storage, &binding.ty),
            BindingLocation::AddressRegister(register) => {
                if !matches!(binding.ty, Type::Ptr(_)) {
                    return Err(Diagnostic::new(format!(
                        "non-pointer local was allocated to address register a{register}"
                    )));
                }
                self.line(&format!("    move.l d1,a{register}"));
                Ok(())
            }
        }
    }

    fn store_d1(&mut self, storage: Storage, ty: &Type) -> Result<(), Diagnostic> {
        let reference = self.storage_ref(storage)?;
        if scalar_width(ty)? == 3 {
            self.line(&format!(
                "    move.b d1,{}",
                self.memory_operand(reference.offset(2))?
            ));
            self.line("    lsr.l #8,d1");
            self.line(&format!(
                "    move.b d1,{}",
                self.memory_operand(reference.offset(1))?
            ));
            self.line("    lsr.l #8,d1");
            self.line(&format!(
                "    move.b d1,{}",
                self.memory_operand(reference)?
            ));
        } else {
            self.line(&format!(
                "    move.{} d1,{}",
                size_suffix(ty)?,
                self.memory_operand(reference)?
            ));
        }
        Ok(())
    }

    fn require_arg_count(
        &self,
        args: &[Expr],
        expected: usize,
        name: &str,
    ) -> Result<(), Diagnostic> {
        if args.len() == expected {
            Ok(())
        } else {
            Err(Diagnostic::new(format!(
                "{name} requires {expected} arguments, got {}",
                args.len()
            )))
        }
    }

    fn intrinsic_constant(
        &self,
        args: &[Expr],
        index: usize,
        requirement: &str,
    ) -> Result<i64, Diagnostic> {
        self.model.const_value(&args[index]).map_err(|_| {
            Diagnostic::new(format!(
                "intrinsic argument {index} requires a constant {requirement}"
            ))
        })
    }

    fn validate_bit_range(
        &self,
        offset: i64,
        width: i64,
        value_width: u8,
    ) -> Result<(u32, u32), Diagnostic> {
        let bits = i64::from(value_width) * 8;
        if offset < 0 || width <= 0 || offset.checked_add(width).map_or(true, |end| end > bits) {
            return Err(Diagnostic::new("bit range is outside the input width"));
        }
        Ok((offset as u32, width as u32))
    }

    fn mask_d0(&mut self, width: u8) {
        let mask = width_mask(width);
        if mask != u32::MAX {
            self.line(&format!("    andi.l #${mask:X},d0"));
        }
    }

    fn mask_register(&mut self, register: &str, width: u8) {
        let mask = width_mask(width);
        if mask != u32::MAX {
            self.line(&format!("    andi.l #${mask:X},{register}"));
        }
    }

    fn emit_immediate_shift(
        &mut self,
        instruction: &str,
        suffix: &str,
        mut count: u32,
        register: &str,
    ) {
        while count > 8 {
            self.line(&format!("    {instruction}.{suffix} #8,{register}"));
            count -= 8;
        }
        if count != 0 {
            self.line(&format!("    {instruction}.{suffix} #{count},{register}"));
        }
    }

    fn extend_register_signed(&mut self, register: &str, ty: &Type) -> Result<(), Diagnostic> {
        match scalar_width(ty)? {
            1 => {
                self.line(&format!("    ext.w {register}"));
                self.line(&format!("    ext.l {register}"));
            }
            2 => self.line(&format!("    ext.l {register}")),
            3 => self.normalize_register_24(register, true),
            4 => {}
            _ => unreachable!(),
        }
        Ok(())
    }

    fn normalize_register_24(&mut self, register: &str, signed: bool) {
        self.line(&format!("    andi.l #$FFFFFF,{register}"));
        if signed {
            let positive = self.next_label("intrinsic_i24_positive");
            self.line(&format!("    btst #23,{register}"));
            self.line(&format!("    beq {positive}"));
            self.line(&format!("    ori.l #$FF000000,{register}"));
            self.line(&format!("{positive}:"));
        }
    }

    fn load_little_24_operands(&mut self, high: &str, middle: &str, low: &str, register: &str) {
        self.line(&format!("    moveq #0,{register}"));
        self.line(&format!("    move.b {high},{register}"));
        self.line(&format!("    lsl.l #8,{register}"));
        self.line(&format!("    move.b {middle},{register}"));
        self.line(&format!("    lsl.l #8,{register}"));
        self.line(&format!("    move.b {low},{register}"));
    }

    fn require_u8_pointer(&self, expr: &Expr) -> Result<(), Diagnostic> {
        let ty = self.model.resolved_type(&self.expr_type(expr)?)?;
        match ty {
            Type::Ptr(inner) if *inner == Type::Named("u8".to_owned()) => Ok(()),
            _ => Err(Diagnostic::new("memory intrinsic requires ptr<u8>")),
        }
    }

    fn reject_known_volatile_memory(
        &self,
        args: &[Expr],
        indexes: &[usize],
    ) -> Result<(), Diagnostic> {
        for index in indexes {
            if self.is_known_volatile_address(&args[*index]) {
                return Err(Diagnostic::new(
                    "this intrinsic requires ordinary nonvolatile memory",
                ));
            }
        }
        Ok(())
    }

    fn is_known_volatile_address(&self, expr: &Expr) -> bool {
        match expr {
            Expr::AddressOf(name) => self
                .model
                .mmio
                .get(name)
                .is_some_and(|(_, _, volatile)| *volatile),
            Expr::AddressOfField { base, .. } => self
                .model
                .mmio
                .get(base)
                .is_some_and(|(_, _, volatile)| *volatile),
            Expr::AddressOfIndex { name, .. } => self
                .model
                .mmio
                .get(name)
                .is_some_and(|(_, _, volatile)| *volatile),
            Expr::Cast { expr, .. } | Expr::BankedPointer { pointer: expr, .. } => {
                self.is_known_volatile_address(expr)
            }
            _ => false,
        }
    }

    fn require_scalar(&self, ty: &Type) -> Result<(), Diagnostic> {
        size_suffix(ty)
            .map(|_| ())
            .map_err(|_| Diagnostic::new("M68k source backend does not support this scalar type"))
    }

    fn storage_ref(&self, storage: Storage) -> Result<MemoryRef, Diagnostic> {
        if let Some(slots) = self.frame_storage_slots.last() {
            for (region, frame_offset) in slots {
                let region_end = region
                    .address
                    .checked_add(region.size)
                    .ok_or_else(|| Diagnostic::new("M68k frame storage address overflow"))?;
                let storage_end = storage
                    .address
                    .checked_add(storage.size)
                    .ok_or_else(|| Diagnostic::new("M68k frame storage address overflow"))?;
                if storage.address >= region.address && storage_end <= region_end {
                    let delta = i32::try_from(storage.address - region.address)
                        .map_err(|_| Diagnostic::new("M68k frame storage offset is too large"))?;
                    return Ok(MemoryRef::Frame(
                        frame_offset
                            .checked_add(delta)
                            .ok_or_else(|| Diagnostic::new("M68k frame displacement overflow"))?,
                    ));
                }
            }
        }
        Ok(MemoryRef::Absolute(storage.address))
    }

    fn memory_operand(&self, reference: MemoryRef) -> Result<String, Diagnostic> {
        Ok(match reference {
            MemoryRef::Absolute(address) => format!("${address:06X}.l"),
            MemoryRef::Frame(offset) => format!("{offset}(a6)"),
            MemoryRef::Stack(offset) => format!("{offset}(sp)"),
        })
    }

    fn load_storage_address(&mut self, storage: Storage, register: &str) -> Result<(), Diagnostic> {
        match self.storage_ref(storage)? {
            MemoryRef::Absolute(address) => {
                self.line(&format!("    move.l #${address:06X},d1"));
                self.line(&format!("    move.l d1,{register}"));
            }
            reference @ (MemoryRef::Frame(_) | MemoryRef::Stack(_)) => {
                self.line(&format!(
                    "    lea {},{}",
                    self.memory_operand(reference)?,
                    register
                ));
            }
        }
        Ok(())
    }

    fn scratch_operand(&self, offset: u32) -> Result<String, Diagnostic> {
        let storage = self.intrinsic_scratch.last().copied().ok_or_else(|| {
            Diagnostic::new("M68k intrinsic scratch is only valid inside a function")
        })?;
        let address = storage
            .address
            .checked_add(offset)
            .ok_or_else(|| Diagnostic::new("M68k intrinsic scratch address overflow"))?;
        self.memory_operand(self.storage_ref(Storage { address, size: 1 })?)
    }

    fn normalize_d0(&mut self, ty: &Type) -> Result<(), Diagnostic> {
        if scalar_width(ty)? != 3 {
            return Ok(());
        }
        self.line("    andi.l #$FFFFFF,d0");
        if type_is_signed(ty) {
            let positive = self.next_label("i24_positive");
            self.line("    move.l d0,d1");
            self.line("    andi.l #$800000,d1");
            self.line(&format!("    beq {positive}"));
            self.line("    ori.l #$FF000000,d0");
            self.line(&format!("{positive}:"));
        }
        Ok(())
    }

    fn bind_memory(&mut self, name: String, storage: Storage, ty: Type) -> Result<(), Diagnostic> {
        let ty = self.model.resolved_type(&ty)?;
        self.bind(
            name,
            Binding {
                location: BindingLocation::Memory(storage),
                ty,
                volatile: false,
            },
        )
    }

    fn bind(&mut self, name: String, binding: Binding) -> Result<(), Diagnostic> {
        if self
            .scopes
            .iter()
            .rev()
            .any(|scope| scope.contains_key(&name))
        {
            return Err(Diagnostic::new(format!(
                "local `{name}` shadows an existing name"
            )));
        }
        self.scopes
            .last_mut()
            .expect("function scope")
            .insert(name, binding);
        Ok(())
    }

    fn binding(&self, name: &str) -> Result<Binding, Diagnostic> {
        if let Some(binding) = self.scopes.iter().rev().find_map(|scope| scope.get(name)) {
            return Ok(binding.clone());
        }
        if let Some(storage) = self.model.globals.get(name) {
            return Ok(Binding {
                location: BindingLocation::Memory(*storage),
                ty: self.model.resolved_type(&self.model.global_types[name])?,
                volatile: false,
            });
        }
        if let Some((address, ty, volatile)) = self.model.mmio.get(name) {
            return Ok(Binding {
                location: BindingLocation::Memory(Storage {
                    address: *address,
                    size: self.model.type_size(ty)?,
                }),
                ty: ty.clone(),
                volatile: *volatile,
            });
        }
        Err(Diagnostic::new(format!("unknown variable `{name}`")))
    }

    fn next_label(&mut self, name: &str) -> String {
        let label = format!("__ezra_{}_{}", sanitize(name), self.labels);
        self.labels += 1;
        label
    }

    fn line(&mut self, line: &str) {
        self.out.push_str(line);
        self.out.push('\n');
    }

    fn emit_data_bytes(&mut self, bytes: Vec<u8>) {
        for line in byte_data_lines("db", &bytes, ByteLiteralStyle::HexPrefix, 16) {
            self.line(&line);
        }
    }

    fn emit_text_data(&mut self, value: &str) {
        self.line(&terminated_text_data_line("db", value, "0"));
    }
}

const M68K_ADDRESS_REGISTERS: [u8; 4] = [2, 3, 4, 5];
const M68K_POINTER_CLASS: RegClass = RegClass(0);
const M68K_MEMORY_CLASS: RegClass = RegClass(1);
const M68K_STACK_SPILL_CLASS: SpillClassId = SpillClassId(0);

fn m68k_local_target() -> Target {
    Target {
        units: M68K_ADDRESS_REGISTERS
            .iter()
            .map(|register| RegisterUnit::new(format!("a{register}")))
            .collect(),
        registers: M68K_ADDRESS_REGISTERS
            .iter()
            .enumerate()
            .map(|(unit, register)| {
                PhysicalRegister::new(format!("a{register}"), vec![RegUnit(unit)])
            })
            .collect(),
        register_classes: vec![
            RegisterClass::new(
                "pointer-address",
                (0..M68K_ADDRESS_REGISTERS.len()).map(PhysReg).collect(),
            ),
            RegisterClass::new("memory-local", Vec::new()),
        ],
        spill_classes: vec![
            SpillClass::new("stack", None, 0)
                .with_base_alignment(2)
                .for_register_classes(vec![M68K_POINTER_CLASS, M68K_MEMORY_CLASS]),
        ],
    }
}

fn allocate_m68k_aligned(model: &mut SemanticModel, size: u32) -> Result<Storage, Diagnostic> {
    if model.next_ram_address() & 1 != 0 {
        model.allocate(1)?;
    }
    model.allocate(size)
}

fn align_even(value: u32) -> u32 {
    (value + 1) & !1
}

fn abi_slot_size(model: &SemanticModel, ty: &Type) -> Result<u32, Diagnostic> {
    Ok(align_even(model.type_size(ty)?))
}

struct FunctionFrame {
    locals: HashMap<String, Binding>,
    frame_size: i32,
    storage_slots: Vec<(Storage, i32)>,
    intrinsic_scratch: Storage,
    aggregate_scratch: Option<Storage>,
}

fn plan_function_locals(
    function: &Function,
    model: &mut SemanticModel,
) -> Result<FunctionFrame, Diagnostic> {
    let mut source_locals = Vec::new();
    let mut local_types = HashMap::new();
    collect_source_locals(&function.body, model, &mut source_locals, &mut local_types)?;
    let clobbers = (0..M68K_ADDRESS_REGISTERS.len())
        .map(PhysReg)
        .collect::<Vec<_>>();
    let planned = allocate_source_locals(
        &m68k_local_target(),
        &source_locals,
        &function.body,
        &clobbers,
    )
    .map_err(regalloc_diagnostic)?;
    let spill_bytes = planned
        .allocation
        .spill_slots
        .iter()
        .map(|slot| slot.offset.saturating_add(slot.size))
        .max()
        .unwrap_or(0);
    let spill_region = (spill_bytes != 0)
        .then(|| allocate_m68k_aligned(model, spill_bytes))
        .transpose()?;

    let aggregate_bytes = local_types
        .values()
        .chain(function.params.iter().map(|param| &param.ty))
        .chain(model.global_types.values())
        .filter_map(|ty| {
            let ty = model.resolved_type(ty).ok()?;
            let aggregate = matches!(ty, Type::Array { .. })
                || matches!(&ty, Type::Named(name) if model.structs.contains_key(name));
            aggregate.then(|| model.type_size(&ty).ok()).flatten()
        })
        .max()
        .unwrap_or(0);

    let mut cursor = spill_bytes;
    cursor = align_even(cursor);
    let intrinsic_scratch = allocate_m68k_aligned(model, 12)?;
    let intrinsic_offset = cursor;
    cursor = cursor
        .checked_add(12)
        .ok_or_else(|| Diagnostic::new("M68k function frame is too large"))?;
    cursor = align_even(cursor);
    let aggregate_scratch = if aggregate_bytes == 0 {
        None
    } else {
        let storage = allocate_m68k_aligned(model, aggregate_bytes)?;
        let offset = cursor;
        cursor = cursor
            .checked_add(aggregate_bytes)
            .ok_or_else(|| Diagnostic::new("M68k function frame is too large"))?;
        Some((storage, offset))
    };
    let frame_size = i32::try_from(align_even(cursor))
        .map_err(|_| Diagnostic::new("M68k function frame is too large"))?;
    if frame_size > i32::from(i16::MAX) + 1 {
        return Err(Diagnostic::new(
            "M68k function frame exceeds the signed LINK displacement",
        ));
    }

    let mut storage_slots = Vec::new();
    if let Some(spill_region) = spill_region {
        storage_slots.push((spill_region, -frame_size));
    }
    storage_slots.push((
        intrinsic_scratch,
        -(frame_size - i32::try_from(intrinsic_offset).unwrap_or(i32::MAX)),
    ));
    let aggregate_scratch_storage = aggregate_scratch.map(|(storage, offset)| {
        storage_slots.push((
            storage,
            -(frame_size - i32::try_from(offset).unwrap_or(i32::MAX)),
        ));
        storage
    });

    let mut bindings = HashMap::new();
    for (name, ty) in local_types {
        let vreg = planned
            .locals
            .vreg(&name)
            .ok_or_else(|| Diagnostic::new(format!("missing allocation for local `{name}`")))?;
        let location = match planned.allocation.location(vreg) {
            Some(Location::Register(register)) => {
                if !matches!(ty, Type::Ptr(_)) {
                    return Err(Diagnostic::new(format!(
                        "non-pointer local `{name}` was allocated to an address register"
                    )));
                }
                let register = *M68K_ADDRESS_REGISTERS.get(register.0).ok_or_else(|| {
                    Diagnostic::new(format!("invalid M68k local register for `{name}`"))
                })?;
                BindingLocation::AddressRegister(register)
            }
            Some(Location::Spill(slot_index)) => {
                let slot = planned
                    .allocation
                    .spill_slots
                    .get(slot_index)
                    .ok_or_else(|| Diagnostic::new(format!("invalid spill slot for `{name}`")))?;
                debug_assert_eq!(slot.class, M68K_STACK_SPILL_CLASS);
                BindingLocation::Memory(Storage {
                    address: spill_region
                        .ok_or_else(|| Diagnostic::new("missing M68k spill frame storage"))?
                        .address
                        + slot.offset,
                    size: slot.size,
                })
            }
            Some(Location::Unused) | None => {
                return Err(Diagnostic::new(format!(
                    "source allocator did not place local `{name}`"
                )));
            }
        };
        bindings.insert(
            name,
            Binding {
                location,
                ty,
                volatile: false,
            },
        );
    }
    Ok(FunctionFrame {
        locals: bindings,
        frame_size,
        storage_slots,
        intrinsic_scratch,
        aggregate_scratch: aggregate_scratch_storage,
    })
}

fn collect_source_locals(
    body: &[Stmt],
    model: &SemanticModel,
    locals: &mut Vec<SourceLocal>,
    local_types: &mut HashMap<String, Type>,
) -> Result<(), Diagnostic> {
    for stmt in body {
        match stmt {
            Stmt::Let { name, ty, .. } => {
                let ty = model.resolved_type(ty)?;
                let pointer = matches!(ty, Type::Ptr(_));
                let class = if pointer {
                    M68K_POINTER_CLASS
                } else {
                    M68K_MEMORY_CLASS
                };
                if local_types.insert(name.clone(), ty.clone()).is_some() {
                    return Err(Diagnostic::new(format!("duplicate local `{name}`")));
                }
                locals.push(
                    SourceLocal::new(name.clone(), model.type_size(&ty)?, 2, class)
                        .with_spill_classes(vec![M68K_STACK_SPILL_CLASS])
                        .with_force_memory(!pointer),
                );
            }
            Stmt::LetTwo {
                first_name,
                first_ty,
                second_name,
                second_ty,
                ..
            } => {
                for (name, source_ty) in [(first_name, first_ty), (second_name, second_ty)] {
                    let ty = model.resolved_type(source_ty)?;
                    let pointer = matches!(ty, Type::Ptr(_));
                    let class = if pointer {
                        M68K_POINTER_CLASS
                    } else {
                        M68K_MEMORY_CLASS
                    };
                    if local_types.insert(name.clone(), ty.clone()).is_some() {
                        return Err(Diagnostic::new(format!("duplicate local `{name}`")));
                    }
                    locals.push(
                        SourceLocal::new(name.clone(), model.type_size(&ty)?, 2, class)
                            .with_spill_classes(vec![M68K_STACK_SPILL_CLASS])
                            .with_force_memory(!pointer),
                    );
                }
            }
            Stmt::If {
                then_body,
                else_body,
                ..
            } => {
                collect_source_locals(then_body, model, locals, local_types)?;
                collect_source_locals(else_body, model, locals, local_types)?;
            }
            Stmt::While { body, .. } | Stmt::Loop { body } => {
                collect_source_locals(body, model, locals, local_types)?;
            }
            _ => {}
        }
    }
    Ok(())
}

fn regalloc_diagnostic(diagnostics: Vec<crate::regalloc::Diagnostic>) -> Diagnostic {
    Diagnostic::new(
        diagnostics
            .into_iter()
            .map(|diagnostic| diagnostic.message)
            .collect::<Vec<_>>()
            .join("; "),
    )
}

fn block_guarantees_two_result_return(body: &[Stmt]) -> bool {
    body.iter().any(stmt_guarantees_two_result_return)
}

fn stmt_guarantees_two_result_return(stmt: &Stmt) -> bool {
    match stmt {
        Stmt::ReturnTwo { .. } => true,
        Stmt::If {
            then_body,
            else_body,
            ..
        } if !else_body.is_empty() => {
            block_guarantees_two_result_return(then_body)
                && block_guarantees_two_result_return(else_body)
        }
        _ => false,
    }
}

fn expr_call_args(expr: &Expr) -> Result<&[Expr], Diagnostic> {
    match expr {
        Expr::Call { args, .. } => Ok(args),
        _ => Err(Diagnostic::new("expected function call")),
    }
}

fn intrinsic_operation(path: &[String]) -> Option<IntrinsicOperation> {
    crate::intrinsics::lookup(&path.join(".")).map(|descriptor| descriptor.operation)
}

fn intrinsic_result_type<F>(
    operation: IntrinsicOperation,
    args: &[Expr],
    mut arg_type: F,
) -> Result<Vec<Type>, Diagnostic>
where
    F: FnMut(&Expr) -> Result<Type, Diagnostic>,
{
    let get = |index: usize, args: &[Expr], arg_type: &mut F| {
        args.get(index)
            .ok_or_else(|| Diagnostic::new("missing intrinsic argument"))
            .and_then(arg_type)
    };
    let named = |name: &str| Type::Named(name.to_owned());
    match operation {
        IntrinsicOperation::Bits(operation) => match operation {
            BitsIntrinsic::Test => Ok(vec![named("bool")]),
            BitsIntrinsic::RotateLeft
            | BitsIntrinsic::RotateRight
            | BitsIntrinsic::Set
            | BitsIntrinsic::Clear
            | BitsIntrinsic::Toggle
            | BitsIntrinsic::Extract
            | BitsIntrinsic::Insert
            | BitsIntrinsic::ByteSwap
            | BitsIntrinsic::Reverse => Ok(vec![get(0, args, &mut arg_type)?]),
            BitsIntrinsic::CountOnes
            | BitsIntrinsic::LeadingZeros
            | BitsIntrinsic::TrailingZeros => Ok(vec![named("u8")]),
        },
        IntrinsicOperation::Int(operation) => match operation {
            IntIntrinsic::WideningMul => {
                let left = get(0, args, &mut arg_type)?;
                let right = get(1, args, &mut arg_type)?;
                let left_info = crate::intrinsics::integer_info(&left).ok_or_else(|| {
                    Diagnostic::new("widening multiply requires integer operands")
                })?;
                let right_info = crate::intrinsics::integer_info(&right).ok_or_else(|| {
                    Diagnostic::new("widening multiply requires integer operands")
                })?;
                if left_info.signed != right_info.signed {
                    return Err(Diagnostic::new(
                        "widening multiply requires matching signedness",
                    ));
                }
                Ok(vec![integer_type_for_bits(
                    left_info.bits + right_info.bits,
                    left_info.signed,
                )?])
            }
            IntIntrinsic::MulHigh | IntIntrinsic::SaturatingAdd | IntIntrinsic::SaturatingSub => {
                Ok(vec![get(0, args, &mut arg_type)?])
            }
            IntIntrinsic::Divmod | IntIntrinsic::FullMul => {
                let ty = get(0, args, &mut arg_type)?;
                Ok(vec![ty.clone(), ty])
            }
            IntIntrinsic::AddCarry | IntIntrinsic::SubBorrow => {
                Ok(vec![get(0, args, &mut arg_type)?, named("bool")])
            }
        },
        IntrinsicOperation::Mem(operation) => match operation {
            MemIntrinsic::CopyNonoverlapping | MemIntrinsic::Move | MemIntrinsic::Fill => {
                Ok(Vec::new())
            }
            MemIntrinsic::FindByte => Ok(vec![Type::Ptr(Box::new(named("u8"))), named("bool")]),
            MemIntrinsic::Compare => Ok(vec![named("i8")]),
            MemIntrinsic::LoadLe16 | MemIntrinsic::LoadBe16 => Ok(vec![named("u16")]),
            MemIntrinsic::LoadLe24 | MemIntrinsic::LoadBe24 => Ok(vec![named("u24")]),
            MemIntrinsic::StoreLe16
            | MemIntrinsic::StoreLe24
            | MemIntrinsic::StoreBe16
            | MemIntrinsic::StoreBe24
            | MemIntrinsic::Poke8 => Ok(Vec::new()),
            MemIntrinsic::Peek8 => Ok(vec![named("u8")]),
        },
    }
}

fn integer_type_for_bits(bits: u16, signed: bool) -> Result<Type, Diagnostic> {
    let name = match (bits, signed) {
        (8, false) => "u8",
        (8, true) => "i8",
        (16, false) => "u16",
        (16, true) => "i16",
        (24, false) => "u24",
        (24, true) => "i24",
        (32, false) => "u32",
        (32, true) => "i32",
        _ => return Err(Diagnostic::new("unsupported intrinsic result width")),
    };
    Ok(Type::Named(name.to_owned()))
}

fn require_unsigned_bit_width(ty: &Type) -> Result<u8, Diagnostic> {
    match ty {
        Type::Named(name) if name == "u8" => Ok(1),
        Type::Named(name) if name == "u16" => Ok(2),
        Type::Named(name) if name == "u24" => Ok(3),
        _ => Err(Diagnostic::new("bit intrinsic requires u8, u16, or u24")),
    }
}

fn contains_function_pointer_program(program: &Program) -> bool {
    program
        .declarations
        .iter()
        .any(|declaration| match declaration {
            Declaration::Global(global) => type_contains_function_pointer(&global.ty),
            Declaration::Function(function) => {
                function
                    .params
                    .iter()
                    .any(|param| type_contains_function_pointer(&param.ty))
                    || function
                        .return_type
                        .as_ref()
                        .is_some_and(type_contains_function_pointer)
                    || function
                        .second_return_type
                        .as_ref()
                        .is_some_and(type_contains_function_pointer)
                    || statements_contain_function_pointer(&function.body)
            }
            _ => false,
        })
}

fn statements_contain_function_pointer(statements: &[Stmt]) -> bool {
    statements.iter().any(|statement| match statement {
        Stmt::Let { ty, .. } => type_contains_function_pointer(ty),
        Stmt::LetTwo {
            first_ty,
            second_ty,
            ..
        } => type_contains_function_pointer(first_ty) || type_contains_function_pointer(second_ty),
        Stmt::If {
            then_body,
            else_body,
            ..
        } => {
            statements_contain_function_pointer(then_body)
                || statements_contain_function_pointer(else_body)
        }
        Stmt::While { body, .. } | Stmt::Loop { body } => statements_contain_function_pointer(body),
        _ => false,
    })
}

fn type_contains_function_pointer(ty: &Type) -> bool {
    match ty {
        Type::Ptr(inner) => matches!(inner.as_ref(), Type::Function { .. }),
        Type::Array { element, .. } => type_contains_function_pointer(element),
        Type::Function {
            params,
            return_type,
        } => {
            params.iter().any(type_contains_function_pointer)
                || return_type
                    .as_ref()
                    .is_some_and(|ty| type_contains_function_pointer(ty))
        }
        Type::Named(_) => false,
    }
}

fn size_suffix(ty: &Type) -> Result<&'static str, Diagnostic> {
    match ty {
        Type::Named(name) if matches!(name.as_str(), "u8" | "i8" | "bool") => Ok("b"),
        Type::Named(name) if matches!(name.as_str(), "u16" | "i16") => Ok("w"),
        Type::Named(name) if matches!(name.as_str(), "u24" | "i24" | "ptr" | "u32" | "i32") => {
            Ok("l")
        }
        Type::Ptr(_) => Ok("l"),
        _ => Err(Diagnostic::new("unsupported M68k scalar type")),
    }
}

fn scalar_width(ty: &Type) -> Result<u8, Diagnostic> {
    match ty {
        Type::Named(name) if matches!(name.as_str(), "u8" | "i8" | "bool") => Ok(1),
        Type::Named(name) if matches!(name.as_str(), "u16" | "i16") => Ok(2),
        Type::Named(name) if matches!(name.as_str(), "u24" | "i24" | "ptr") => Ok(3),
        Type::Named(name) if matches!(name.as_str(), "u32" | "i32") => Ok(4),
        Type::Ptr(_) => Ok(3),
        _ => Err(Diagnostic::new("unsupported M68k scalar type")),
    }
}

fn width_mask(width: u8) -> u32 {
    match width {
        1 => 0xFF,
        2 => 0xFFFF,
        3 => 0xFF_FFFF,
        4 => 0xFFFF_FFFF,
        _ => unreachable!(),
    }
}

fn should_use_memory_shift(count: u32) -> bool {
    if count == 0 {
        return false;
    }
    let memory_cycles = 20 * count;
    let register_cycles = 28 + 8 + 2 * count;
    memory_cycles < register_cycles
}

fn match_rotate<'a, F>(expr: &'a Expr, bits: u32, shift_count: F) -> Option<(&'a Expr, u32)>
where
    F: Fn(&Expr) -> Option<u32>,
{
    let Expr::Binary {
        left,
        op: BinaryOp::BitOr,
        right,
    } = expr
    else {
        return None;
    };
    let shifted = |expr: &'a Expr| {
        let Expr::Binary { left, op, right } = expr else {
            return None;
        };
        matches!(op, BinaryOp::Shl | BinaryOp::Shr)
            .then(|| (left.as_ref(), *op, shift_count(right)))
            .and_then(|(source, op, count)| count.map(|count| (source, op, count)))
    };
    let first = shifted(left)?;
    let second = shifted(right)?;
    let (left_shift, right_shift) = match (first.1, second.1) {
        (BinaryOp::Shl, BinaryOp::Shr) => (first, second),
        (BinaryOp::Shr, BinaryOp::Shl) => (second, first),
        _ => return None,
    };
    (left_shift.0 == right_shift.0
        && left_shift.2 != 0
        && right_shift.2 != 0
        && left_shift.2 < bits
        && left_shift.2 + right_shift.2 == bits)
        .then_some((left_shift.0, left_shift.2))
}

fn flatten_bit_or<'a>(expr: &'a Expr, terms: &mut Vec<&'a Expr>) {
    if let Expr::Binary {
        left,
        op: BinaryOp::BitOr,
        right,
    } = expr
    {
        flatten_bit_or(left, terms);
        flatten_bit_or(right, terms);
    } else {
        terms.push(expr);
    }
}

fn shifted_mask_term<'a, F>(
    expr: &'a Expr,
    shift_count: F,
) -> Option<(&'a Expr, BinaryOp, u32, Option<u64>)>
where
    F: Fn(&Expr) -> Option<u32>,
{
    let Expr::Binary { left, op, right } = expr else {
        return None;
    };
    if !matches!(op, BinaryOp::Shl | BinaryOp::Shr) {
        return None;
    }
    let count = shift_count(right)?;
    let (source, mask) = match left.as_ref() {
        Expr::Binary {
            left: source,
            op: BinaryOp::BitAnd,
            right: mask,
        } => (
            source.as_ref(),
            Some(u64::try_from(const_expr_value(mask)?).ok()?),
        ),
        _ => (left.as_ref(), None),
    };
    Some((source, *op, count, mask))
}

fn const_expr_value(expr: &Expr) -> Option<i64> {
    match expr {
        Expr::Int(value) | Expr::TypedInt(value, _) => Some(*value),
        _ => None,
    }
}

fn match_u16_byte_swap<'a, F>(terms: &[&'a Expr], shift_count: F) -> Option<&'a Expr>
where
    F: Fn(&Expr) -> Option<u32> + Copy,
{
    if terms.len() != 2 {
        return None;
    }
    let mut left_source = None;
    let mut right_source = None;
    for term in terms {
        let (source, op, count, mask) = shifted_mask_term(term, shift_count)?;
        if count != 8 {
            return None;
        }
        match (op, mask) {
            (BinaryOp::Shl, None | Some(0xFF) | Some(0xFFFF)) if left_source.is_none() => {
                left_source = Some(source)
            }
            (BinaryOp::Shr, None | Some(0xFF00) | Some(0xFFFF)) if right_source.is_none() => {
                right_source = Some(source)
            }
            _ => return None,
        }
    }
    let left_source = left_source?;
    let right_source = right_source?;
    (left_source == right_source).then_some(left_source)
}

fn match_u32_byte_swap<'a, F>(terms: &[&'a Expr], shift_count: F) -> Option<&'a Expr>
where
    F: Fn(&Expr) -> Option<u32> + Copy,
{
    if terms.len() != 4 {
        return None;
    }
    let expected = [
        (BinaryOp::Shl, 24_u32, 0xFF_u64),
        (BinaryOp::Shl, 8_u32, 0xFF00_u64),
        (BinaryOp::Shr, 8_u32, 0xFF0000_u64),
        (BinaryOp::Shr, 24_u32, 0xFF000000_u64),
    ];
    let mut source = None;
    let mut found = [false; 4];
    for term in terms {
        let (term_source, op, count, Some(mask)) = shifted_mask_term(term, shift_count)? else {
            return None;
        };
        let Some(index) =
            expected
                .iter()
                .position(|(expected_op, expected_count, expected_mask)| {
                    *expected_op == op && *expected_count == count && *expected_mask == mask
                })
        else {
            return None;
        };
        if found[index] {
            return None;
        }
        found[index] = true;
        if let Some(source) = source {
            if source != term_source {
                return None;
            }
        } else {
            source = Some(term_source);
        }
    }
    found.into_iter().all(|found| found).then_some(source?)
}

fn element_type(ty: &Type) -> Result<Type, Diagnostic> {
    match ty {
        Type::Array { element, .. } | Type::Ptr(element) => Ok((**element).clone()),
        _ => Err(Diagnostic::new("indexing requires array or pointer")),
    }
}

fn type_is_signed(ty: &Type) -> bool {
    matches!(ty, Type::Named(name) if name.starts_with('i'))
}
fn is_comparison(op: BinaryOp) -> bool {
    matches!(
        op,
        BinaryOp::Eq | BinaryOp::Ne | BinaryOp::Lt | BinaryOp::Le | BinaryOp::Gt | BinaryOp::Ge
    )
}
fn comparison_branch(op: BinaryOp, signed: bool) -> &'static str {
    match (op, signed) {
        (BinaryOp::Eq, _) => "beq",
        (BinaryOp::Ne, _) => "bne",
        (BinaryOp::Lt, true) => "blt",
        (BinaryOp::Le, true) => "ble",
        (BinaryOp::Gt, true) => "bgt",
        (BinaryOp::Ge, true) => "bge",
        (BinaryOp::Lt, false) => "bcs",
        (BinaryOp::Le, false) => "bls",
        (BinaryOp::Gt, false) => "bhi",
        (BinaryOp::Ge, false) => "bcc",
        _ => unreachable!(),
    }
}
fn assign_binary(op: AssignOp) -> BinaryOp {
    match op {
        AssignOp::Add => BinaryOp::Add,
        AssignOp::Sub => BinaryOp::Sub,
        AssignOp::Mul => BinaryOp::Mul,
        AssignOp::Div => BinaryOp::Div,
        AssignOp::Mod => BinaryOp::Mod,
        AssignOp::BitAnd => BinaryOp::BitAnd,
        AssignOp::BitOr => BinaryOp::BitOr,
        AssignOp::BitXor => BinaryOp::BitXor,
        AssignOp::Shl => BinaryOp::Shl,
        AssignOp::Shr => BinaryOp::Shr,
        AssignOp::Set => unreachable!(),
    }
}
fn function_label(name: &str) -> String {
    format!("_{}", sanitize(name))
}
fn sanitize(name: &str) -> String {
    name.chars()
        .map(|ch| {
            if ch.is_ascii_alphanumeric() || ch == '_' {
                ch
            } else {
                '_'
            }
        })
        .collect()
}

#[cfg(test)]
mod tests;
