...
//!
//! Generated functions use near calls and BP-based stack frames. Scalar results
//! are written through caller-provided hidden result pointers, while `r0` is a
//! per-invocation compiler scratch area. Pointers are 16-bit offsets in the
//! single segment established by the startup stub.
...
impl FrameState {
    fn new() -> Self {
        Self {
            next_offset: I8086_FRAME_SCRATCH_END,
            max_offset: I8086_FRAME_SCRATCH_END,
        }
    }
}
...
struct Emitter {
    ...
    return_pointers: Vec<Option<Storage>>,
...
impl Emitter {
    fn new(model: SemanticModel, options: AssemblyOptions) -> Result<Self, Diagnostic> {
        Ok(Self {
            ...
            ports: HashMap::new(),
            r0: frame_storage(I8086_R0_OFFSET, SCRATCH_BYTES),
            r1: frame_storage(I8086_R1_OFFSET, SCRATCH_BYTES),
            r2: frame_storage(I8086_R2_OFFSET, SCRATCH_BYTES),
        })
    }
    ...
    fn emit(mut self, program: &Program) -> Result<String, Diagnostic> {
        ...
        self.line("    push bp");
        self.line("    mov bp,sp");
        let static_frame_adjustment = self.reserve_stack_adjustment();
        self.frame_states.push(FrameState::new());
        self.emit_static_initializers(program)?;
        let static_frame = self
            .frame_states
            .pop()
            .expect("static initializer frame state");
        self.patch_stack_adjustment(static_frame_adjustment, static_frame.max_offset);
        self.line(&format!("    add sp,{}", imm(static_frame.max_offset)));
        self.line("    pop bp");
    ...
    fn emit_function(&mut self, function: &Function) -> Result<(), Diagnostic> {
        ...
        {
            return Err(Diagnostic::new(
                "main cannot return a value because its startup caller has no result destination",
            ));
        }
        ...
        self.return_pointers.push(None);
        self.second_return_pointers.push(None);
        self.function_states.push(FunctionState { naked });
        ...
        self.line("    push bp");
        self.line("    mov bp,sp");
        let frame_adjustment = self.reserve_stack_adjustment();
        self.frame_states.push(FrameState::new());
        ...
        let mut hidden_offset = 4;
        if function.return_type.is_some() {
            let pointer = self.allocate(2)?;
            self.copy(frame_argument_storage(hidden_offset, 2), pointer, 2);
            self.return_pointers
                .last_mut()
                .expect("return pointer stack")
                .replace(pointer);
            hidden_offset += 2;
        }
        if function.second_return_type.is_some() {
            let pointer = self.allocate(2)?;
            self.copy(frame_argument_storage(hidden_offset, 2), pointer, 2);
            self.second_return_pointers
                .last_mut()
                .expect("second return pointer stack")
                .replace(pointer);
            hidden_offset += 2;
        }
        for (param, ty) in function.params.iter().zip(&signature.params) {
            ...
            self.bind(
                param.name.clone(),
                BindingLocation::Frame(frame_argument_storage(hidden_offset, size)),
                param.ty.clone(),
            )?;
            hidden_offset = hidden_offset
                .checked_add(self.abi_slot_size(ty)?)
                .ok_or_else(|| Diagnostic::new("8086 parameter frame is too large"))?;
        }

        let spill_storage = local_plan
            .spill_sizes
            .iter()
            .map(|size| self.allocate(*size))
            .collect::<Result<Vec<_>, _>>()?;
        for (name, planned) in local_plan.bindings {
            let storage = match planned.location {
                PlannedLocation::Spill(slot) => *spill_storage
                    .get(slot)
                    .ok_or_else(|| Diagnostic::new("invalid i8086 frame spill slot"))?,
            };
            self.bind(name, BindingLocation::Frame(storage), planned.ty)?;
        }
        self.emit_block(&function.body)?;
        self.line(&format!("{return_label}:"));
        let frame = self.frame_states.pop().expect("function frame state");
        if frame.max_offset > I8086_MAX_FRAME_BYTES {
            return Err(Diagnostic::new(
                "8086 function frame exceeds the signed BP displacement range",
            ));
        }
        self.patch_stack_adjustment(frame_adjustment, frame.max_offset);
        self.line(&format!("    add sp,{}", imm(frame.max_offset)));
        self.line("    pop bp");
        if interrupt {
            self.line("    pop es");
            self.line("    pop ds");
    ...
    fn emit_stmt(&mut self, stmt: &Stmt) -> Result<(), Diagnostic> {
        ...
        match stmt {
            ...
            Stmt::LetTwo {
                ...
                let first_storage = self.static_storage(&first)?;
            ...
            Stmt::Assign { target, op, value } => {
                ...
                let Ok(width) = self.scalar_width(&ty) else {
                    ...
                    let size = self.model.type_size(&ty)?;
                    let temporary = self.allocate(size)?;
                ...
                if *op == AssignOp::Set {
                    ...
                    if !immediate {
                        let left = self.allocate(u32::from(width))?;
                ...
                let result = self.allocate(u32::from(width))?;
            ...
            Stmt::Return(value) => {
                ...
                match (value, return_type) {
                    (Some(value), Some(ty)) => {
                        self.emit_expr(value, &ty)?;
                        self.store_return_result(self.scalar_width(&ty)?)?;
                    }
                    (Some(_), None) => {
    ...
    fn emit_binding_initializer(
        ...
        self.emit_initializer(self.static_storage(binding)?, ty, value)
    }
    ...
    fn emit_condition(
        ...
        match condition {
            ...
            Expr::Binary { left, op, right } if is_comparison(*op) => {
                ...
                let left_value = self.allocate(u32::from(width))?;
        ...
        match expr {
            ...
            Expr::AddressOf(name) => {
                    ...
                    .is_some_and(|signature| signature.second_return_type.is_some())
                {
                    ...
                    let binding = self.binding(name)?;
                    self.load_storage_address_into_result(
                        self.static_storage(&binding)?.address,
                        width,
                    );
                }
            }
            ...
            Expr::AddressOfField { base, field } => {
                ...
                match address {
                    Address::Direct(address) => {
                        self.load_storage_address_into_result(address, width)
                    }
            ...
            Expr::Binary { left, op, right } => {
                ...
                if !immediate {
                    let left_value = self.allocate(u32::from(operand_width))?;
    ...
    fn store_result_binding(&mut self, binding: &Binding) -> Result<(), Diagnostic> {
        ...
        self.copy(self.r0, self.static_storage(binding)?, u32::from(width));
        Ok(())
    }
    ...
    fn emit_two_result_call(
        ...
        if second_destination.size < u32::from(second_width)
            || first_destination.size < u32::from(self.scalar_width(signature_first)?)
        {
            return Err(Diagnostic::new(
                "two-result destination is too small for the function result",
            ));
        }
        let values = self.evaluate_call_arguments(args, &signature.params)?;
        let mut argument_bytes: u32 = 0;
        for (value, ty) in values.iter().zip(&signature.params).rev() {
            self.push_storage_argument(*value, self.abi_slot_size(ty)?)?;
            argument_bytes = argument_bytes
                .checked_add(self.abi_slot_size(ty)?)
                .ok_or_else(|| Diagnostic::new("8086 argument frame is too large"))?;
        }
        self.push_address(second_destination.address);
        self.push_address(first_destination.address);
        argument_bytes = argument_bytes
            .checked_add(4)
            .ok_or_else(|| Diagnostic::new("8086 result frame is too large"))?;
        self.line(&format!("    call near {}", function_label(&resolved)));
        self.adjust_stack(argument_bytes);
        self.copy(first_destination, self.r0, first_destination.size);
        self.extend_result(
    ...
    fn emit_return_two(&mut self, first: &Expr, second: &Expr) -> Result<(), Diagnostic> {
        ...
        let Some(first_pointer) = self.return_pointers.last().copied().flatten() else {
            return Err(Diagnostic::new(
                "two-result function has no first result destination",
            ));
        };
        let Some(second_pointer) = self.second_return_pointers.last().copied().flatten() else {
            return Err(Diagnostic::new(
                "two-result function has no second result destination",
            ));
        };
        let first_value = self.allocate(self.model.type_size(&first_type)?)?;
        ...
        self.load_bx(second_pointer.address);
        self.copy_storage_to_indirect(self.r0, u32::from(second_width));
        self.load_bx(first_pointer.address);
        self.copy_storage_to_indirect(first_value, first_value.size);
    ...
    fn emit_call(
        ...
        let result_destination = signature
            .return_type
            .as_ref()
            .map(|ty| self.allocate(self.model.type_size(ty)?))
            .transpose()?;
        let values = self.evaluate_call_arguments(args, &signature.params)?;
        let mut argument_bytes: u32 = 0;
        for (value, ty) in values.iter().zip(&signature.params).rev() {
            let slot_size = self.abi_slot_size(ty)?;
            self.push_storage_argument(*value, slot_size)?;
            argument_bytes = argument_bytes
                .checked_add(slot_size)
                .ok_or_else(|| Diagnostic::new("8086 argument frame is too large"))?;
        }
        if let Some(destination) = result_destination {
            self.push_address(destination.address);
            argument_bytes = argument_bytes
                .checked_add(2)
                .ok_or_else(|| Diagnostic::new("8086 result frame is too large"))?;
        }
        ...
        self.adjust_stack(argument_bytes);
        if let Some(destination) = result_destination {
            let return_type = signature.return_type.as_ref().expect("result type");
            self.copy(destination, self.r0, destination.size);
            self.extend_result(
                self.scalar_width(return_type)?,
                self.scalar_width(expected)?,
                self.type_is_signed(return_type)?,
            );
        }
        Ok(())
    }
    ...
    fn intrinsic_arguments(
        ...
        resolution
            ...
            .zip(args)
            .map(|(ty, arg)| {
                let storage = self.allocate(self.model.type_size(ty)?)?;
    ...
    fn lower_int_intrinsic(
        ...
        match operation {
            ...
            IntIntrinsic::Divmod => {
                ...
                let quotient = self.allocate(u32::from(width))?;
                self.copy(values[0], self.r0, u32::from(width));
                self.copy(values[1], self.r1, u32::from(width));
    ...
    fn intrinsic_count_bits(&mut self, source: Storage, width: u8, leading: bool) {
        let input = self.allocate(u32::from(width)).expect("count input");
        self.copy(source, input, u32::from(width));
        self.zero(self.r0);
    ...
    fn intrinsic_rotate(
        ...
        let remaining = self.allocate(count.size)?;
        self.copy(count, remaining, count.size);
        let one = self.allocate(count.size)?;
    ...
    fn intrinsic_full_product(
        ...
        let product = self.allocate(u32::from(product_width))?;
        let multiplicand = self.allocate(u32::from(product_width))?;
        let multiplier = self.allocate(u32::from(right_width))?;
        let negative = self.allocate(1)?;
        self.zero(product);
    ...
    fn divide(&mut self, width: u8, remainder: bool, signed: bool) -> Result<(), Diagnostic> {
        ...
        let quotient_negative = self.allocate(1)?;
        let remainder_negative = self.allocate(1)?;
        ...
        let divisor = self.allocate(u32::from(width))?;
        let remainder_value = self.allocate(u32::from(width))?;
    ...
    fn named_index_address(&mut self, name: &str, index: &Expr) -> Result<Type, Diagnostic> {
        ...
        match (root, &resolved) {
            (AccessRoot::Storage(binding), Type::Array { len, .. }) => {
                ...
                self.load_storage_address_into_bx(self.static_storage(&binding)?.address);
            }
    ...
    fn emit_pointer_add(
        ...
        if pointer_on_left {
            ...
            let index_width = self.scalar_width(&index_ty)?;
            self.emit_expr(index, &index_ty)?;
            let saved_index = self.allocate(u32::from(index_width))?;
    ...
    fn allocate(&mut self, size: u32) -> Result<Storage, Diagnostic> {
        let frame = self
            .frame_states
            .last_mut()
            .ok_or_else(|| Diagnostic::new("i8086 compiler storage requested outside a frame"))?;
        let offset = frame
            .next_offset
            .checked_add(1)
            .map(|value| value & !1)
            .ok_or_else(|| Diagnostic::new("8086 function frame is too large"))?;
        let end = offset
            .checked_add(size)
            .ok_or_else(|| Diagnostic::new("8086 function frame is too large"))?;
        frame.next_offset = end;
        frame.max_offset = frame.max_offset.max(end);
        Ok(frame_storage(offset, size))
    }

    fn abi_slot_size(&self, ty: &Type) -> Result<u32, Diagnostic> {
        let size = self.model.type_size(ty)?;
        if size > 4 {
            return Err(Diagnostic::new(
                "8086 scalar argument is too wide for the stack ABI",
            ));
        }
        Ok(size.checked_add(1).map(|value| value & !1).unwrap_or(size))
    }

    fn reserve_stack_adjustment(&mut self) -> usize {
        let position = self.out.len();
        self.line("    sub sp,0");
        position
    }

    fn patch_stack_adjustment(&mut self, position: usize, size: u32) {
        let old = "    sub sp,0\n";
        let replacement = format!("    sub sp,{}\n", imm(size));
        self.out
            .replace_range(position..position + old.len(), &replacement);
    }

    fn adjust_stack(&mut self, bytes: u32) {
        if bytes != 0 {
            self.line(&format!("    add sp,{}", imm(bytes)));
        }
    }
    ...
    fn evaluate_call_arguments(
        ...
        for (arg, ty) in args.iter().zip(params) {
            let value = self.allocate(self.model.type_size(ty)?)?;
            self.emit_initializer(value, ty, arg)?;
            values.push(value);
        }
        Ok(values)
    }

    fn push_storage_argument(
        &mut self,
        storage: Storage,
        slot_size: u32,
    ) -> Result<(), Diagnostic> {
        if slot_size < storage.size || !slot_size.is_multiple_of(2) {
            return Err(Diagnostic::new("invalid 8086 stack argument size"));
        }
        for offset in (0..slot_size).step_by(2).rev() {
            if offset + 1 < storage.size {
                self.load_ax(storage.address + offset);
            } else if offset < storage.size {
                self.load_al(storage.address + offset);
                self.line("    xor ah,ah");
            } else {
                self.line("    xor ax,ax");
    ...
    fn push_address(&mut self, address: u32) {
        self.load_storage_address_into_ax(address);
        self.line("    push ax");
    }

    fn load_storage_address_into_ax(&mut self, address: u32) {
        if let Some(offset) = frame_local_offset(address) {
            self.line("    mov ax,bp");
            self.line(&format!("    sub ax,{}", imm(offset)));
        } else if let Some(offset) = frame_argument_offset(address) {
            self.line("    mov ax,bp");
            if offset != 0 {
                self.line(&format!("    add ax,{}", imm(offset)));
            }
        } else {
            self.line(&format!("    mov ax,{}", imm(address)));
        }
    }

    fn load_storage_address_into_bx(&mut self, address: u32) {
        if let Some(offset) = frame_local_offset(address) {
            self.line("    mov bx,bp");
            self.line(&format!("    sub bx,{}", imm(offset)));
        } else if let Some(offset) = frame_argument_offset(address) {
            self.line("    mov bx,bp");
            if offset != 0 {
                self.line(&format!("    add bx,{}", imm(offset)));
    ...
    fn load_storage_address_into_result(&mut self, address: u32, width: u8) {
        self.load_storage_address_into_ax(address);
        self.line(&format!("    mov {},ax", mem(self.r0.address)));
        if width > 2 {
            self.line("    xor ax,ax");
    ...
    fn store_return_result(&mut self, width: u8) -> Result<(), Diagnostic> {
        let pointer = self
            .return_pointers
            .last()
            .and_then(Clone::clone)
            .ok_or_else(|| Diagnostic::new("value-returning function has no result destination"))?;
        self.load_bx(pointer.address);
        self.copy_storage_to_indirect(self.r0, u32::from(width));
        Ok(())
    }

    fn static_storage(&self, binding: &Binding) -> Result<Storage, Diagnostic> {
        Ok(match binding.location {
            BindingLocation::Static(storage) | BindingLocation::Frame(storage) => storage,
        })
    }

    fn copy_binding_to_storage(&mut self, binding: &Binding, target: Storage, size: u32) {
        let source = self
            .static_storage(binding)
            .expect("binding storage must be addressable");
    ...
    fn scale(&mut self, storage: Storage, width: u8, amount: u32) -> Result<(), Diagnostic> {
        ...
        let source = self.allocate(u32::from(width))?;
        let result = self.allocate(u32::from(width))?;
...
const I8086_FRAME_SPILL_CLASS: SpillClassId = SpillClassId(0);
const I8086_BP_REGISTER: PhysReg = PhysReg(14);
const I8086_FRAME_LOCAL_BASE: u32 = 0x1_0000;
const I8086_FRAME_ARGUMENT_BASE: u32 = 0x2_0000;
const I8086_R0_OFFSET: u32 = 2;
const I8086_R1_OFFSET: u32 = I8086_R0_OFFSET + SCRATCH_BYTES;
const I8086_R2_OFFSET: u32 = I8086_R1_OFFSET + SCRATCH_BYTES;
const I8086_FRAME_SCRATCH_END: u32 = I8086_R2_OFFSET + SCRATCH_BYTES;
const I8086_MAX_FRAME_BYTES: u32 = 0x8000;
...
fn i8086_local_target() -> Target {
    Target {
        ...
        register_classes: vec![
            RegisterClass::new("local-word", vec![]),
            RegisterClass::new("byte", vec![]),
        ],
        spill_classes: vec![SpillClass::new("frame", None, 1).with_base_alignment(1)],
    }
}
...
fn plan_function_locals(
    ...
    let spill_sizes = allocation
        ...
        .iter()
        .map(|slot| {
            debug_assert_eq!(slot.class, I8086_FRAME_SPILL_CLASS);
            slot.size
        })
        .collect();
...
fn collect_i8086_local(
    ...
    locals.push(
        ...
        .with_spill_classes(vec![I8086_FRAME_SPILL_CLASS])
        .with_force_memory(
...
fn frame_storage(offset: u32, size: u32) -> Storage {
    Storage {
        address: I8086_FRAME_LOCAL_BASE + offset,
        size,
    }
}

fn frame_argument_storage(offset: u32, size: u32) -> Storage {
    Storage {
        address: I8086_FRAME_ARGUMENT_BASE + offset,
        size,
    }
}

fn frame_local_offset(address: u32) -> Option<u32> {
    address
        .checked_sub(I8086_FRAME_LOCAL_BASE)
        .filter(|offset| *offset <= I8086_MAX_FRAME_BYTES)
}

fn frame_argument_offset(address: u32) -> Option<u32> {
    address
        .checked_sub(I8086_FRAME_ARGUMENT_BASE)
        .filter(|offset| *offset <= I8086_MAX_FRAME_BYTES)
}
