...
fn fixed_operand_reserves_aliased_register_before_it_starts() {
    ...
    let function = Function::new(
        ...
        vec![BasicBlock::new(
            vec![
                Instruction::new().with_defs(vec![VReg(0)]),
                Instruction::new().with_fixed_defs(vec![FixedOperand::new(VReg(1), PhysReg(0))]),
                Instruction::new().with_uses(vec![VReg(0), VReg(1)]),
            ],
            vec![],
        )],
...
fn forced_spill_with_a_fixed_operand_is_a_diagnostic() {
    ...
    let function = Function::new(
        ...
        vec![BasicBlock::new(
            vec![Instruction::new().with_fixed_defs(vec![FixedOperand::new(VReg(0), PhysReg(0))])],
            vec![],
        )],
    );
...
fn conflicting_fixed_operands_are_diagnostics() {
    ...
    let function = Function::new(
        ...
        vec![BasicBlock::new(
            vec![
                Instruction::new().with_fixed_defs(vec![FixedOperand::new(VReg(0), PhysReg(0))]),
                Instruction::new().with_fixed_uses(vec![FixedOperand::new(VReg(0), PhysReg(3))]),
            ],
            vec![],
        )],
    );
