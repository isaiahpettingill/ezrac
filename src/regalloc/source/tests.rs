...
fn builds_while_and_loop_break_continue_edges() {
    ...
    let lowered =
        lower_source_function(&[local("condition")], &body, &[]).expect("lowering should succeed");
...
fn address_taken_and_caller_forced_locals_spill() {
    ...
    let result =
        allocate_source_locals(&target(), &locals, &body, &[]).expect("allocation should succeed");
...
fn asm_names_are_effects_and_asm_is_opaque() {
    ...
    let lowered = lower_source_function(&[local("input"), local("output")], &body, &[PhysReg(0)])
        .expect("lowering should succeed");
    let instruction = &lowered.function.blocks[0].instructions[0];
